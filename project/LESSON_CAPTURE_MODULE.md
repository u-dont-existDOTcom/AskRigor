...
3. require a successful opaque incident receipt before generalized submission;
...
If incident capture fails for a current lesson incident, fail closed. Do not
call `submit_lesson_candidate`, do not claim lesson archival, and report that
the exact incident was not preserved. Keep the already-authorized pending
candidate available for a retry in the same chat unless the user declines,
changes subject, or stops lesson submission. Historical recovery is different:
when exact historical turns genuinely no longer exist, retain the truthful
partial or lesson-only status without fabricating missing turns.
...
- `Yes` authorizes exactly the currently displayed candidate and its minimum
  ...
  after a successful opaque incident receipt, call `submit_lesson_candidate`
  once with `consent_scope: "once"`; then clear the pending candidate without
  enabling standing consent. If capture fails, do not call the generalized
  submission and retain the pending candidate only for a same-chat retry.
- `Yes always in this chat` authorizes the displayed candidate and its incident
  window and enables standing consent only in the current chat. Perform private
  incident capture first; submit the generalized lesson only after a successful
  opaque incident receipt. A capture failure does not disable standing consent,
  but it does block that candidate's generalized submission until a successful
  same-chat retry.
...
- Clear the pending candidate after the initial `Yes always in this chat`
  submission when it succeeds. Clear it after every later standing-consent capture.
  A failed private incident capture is not a completed standing-consent capture
  and leaves the pending candidate available for retry.
