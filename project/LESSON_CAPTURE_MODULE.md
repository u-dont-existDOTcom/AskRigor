...
## Eligibility and candidate display
...
Do not propose or submit a candidate when the criticism is unverified, is a preference disagreement, or remains in doubt. The candidate must be a general
product lesson that stands without the individual user's facts.
...
Do not store, transmit, or reconstruct this state on the AskRigor server. After
every call, display the returned receipt after the already displayed candidate,
including for later standing-consent submissions.
...
Map the returned status exactly. Never claim success before a success status,
and never convert a failure into a success. Use `retryable` only to state
truthfully whether the user may retry.
