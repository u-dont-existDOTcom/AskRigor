...
4. For every shortlisted creator-content candidate, call `get_youtube_video`, then `get_youtube_transcript`. Continue transcript cursors until the selected track reports `api_visible_complete` or a terminal access boundary. Preserve language, automatic-caption flag, timestamps, pagination, and `access_status`. Titles/descriptions are discovery metadata; comments describe the discussion; neither establishes the video's creator content. If transcript access fails, do not say what the video teaches or recommends. Transcript retrieval verifies wording, not efficacy, accuracy, or causality.
