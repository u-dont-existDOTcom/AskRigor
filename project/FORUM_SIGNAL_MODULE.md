...
4. For a broad treatment/avoid-surgery question with a substantial YouTube corpus, require a `gemini_youtube_candidate_handoff` from the configured high-recall scout. Do not call the lane complete merely because the skill or validator exists; without a supplied and reconciled frontier, block broad synthesis. Call `validate_gemini_youtube_candidate_handoff` when advertised; otherwise validate each identity with `get_youtube_video`. Preserve the complete validated candidate-frontier receipt and reconcile every validated ID; caller materiality or redundancy labels cannot waive screening. Only literal not-found/not-visible results or identity mismatches after complete provider retrieval terminally reject a lead. Credential, configuration, access, malformed-response, runtime, rate-limit, and other non-identity failures remain unresolved regardless of immediate retryability. Preserve each program, population/stage, outcome/horizon, and summary as provisional discovery annotations. When AskRigor cannot retrieve captions, say plainly that the scout's video summary was not checked against a transcript; keep its query and candidate-discovery value, but do not use it as proof of creator content, efficacy, safety, causality, comparison, structure, or a recommendation. Never accept an invented or mismatched identifier.
