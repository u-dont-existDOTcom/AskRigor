...
For broad treatment/avoid-surgery with substantial YouTube results, require supplied `gemini_youtube_candidate_handoff`; call `validate_gemini_youtube_candidate_handoff`. If it, `get_youtube_transcript`, or `assess_treatment_landscape_coverage` is absent, say Action setup is out of date and stop; no substitute/final comparison. Screen all leads. Without captions say the summary was not checked against a transcript; never use it as evidence.
