...
| Exact frontier miss is presented as proof that no evidence exists | The public envelope uses `not_indexed`, a fixed no-negative-evidence message, and an empty typed frontier projection | Continue fresh discovery; no evidence, efficacy, or safety inference is permitted |
| Old frontier is presented as current merely because retrieval succeeded | Every response fixes `frontier_currency:not_assessed` and exposes exact confirmed windows, gaps, partial/blocked passes, and unresolved trails | Revalidate current source, protocol, access, and correction state and execute delta work before synthesis |
| Repository failure leaks credentials or internal state | Public failures use fixed allowlisted codes/messages and discard raw exception text; output schemas contain no connection or provider-body field | Return sanitized `repository_unavailable` or retryable `error`; continue other executable work |
| Public frontier lookup mutates canonical evidence | The operation depends on a one-method reader interface; PostgreSQL starts a repeatable-read read-only transaction; runtime credentials retain SELECT only and default read-only | Mutation is impossible through the operation; deployment acceptance fails on any write grant |
...
| Appeal reversal mutates the original conduct or scientific record | Append-only appeal rows bind distinct original and resolution moderation events on the same target, preserve source-meaning hash, and require scientific disposition unchanged | Service/database reject nonappealable originals, target/action mismatch, in-place resolution, meaning change, or scientific authority |
| Community volume downgrades or upgrades formal evidence | Formal-evidence updates bind contiguous cluster versions, exact scope relationship, freshness state, and evidence identifiers; report-count changes are explicitly non-evidentiary and originating reports remain | Contract/database reject volume-driven evidence, report erasure, meaning change, or percentage display |
| Popularity overrides answerability or an answered scope | Feasibility assessments bind the exact proposal, question, and evidence check; answered scopes and infeasible designs have distinct blocked states, while all other states still require methods/ethics review | Contract/service/database reject mismatched dependencies, wrong disposition, launch, or recruitment |
| Negative/null research result disappears or punishes its origin | Closed-loop result receipts bind exact proposal/question, cluster, lead, and forum origins while requiring report retention, no hypothesis penalty, no meaning rewrite, no causal claim, and no efficacy percentage | Service/database reject missing origins, invented targets, erasure, penalty, causal upgrade, or recruitment |
| Rare facts remain reidentifying after a superficial rewrite | A lead/version-bound gate records exact before/after risk sets, requires explicit generalization when they change, and forbids ordinary projection while any residual risk remains | Contract/service/database reject an eligible decision with residual risk, introduced risk, mismatched dependency, or false generalization state |
| Minor or unknown-age status bypasses enhanced review | The gate binds the lead's exact minor status and requires guardian plus legal/privacy states; minor and unknown-age fixtures cannot enter ordinary projection even when direct identifiers are absent | Service/database reject an adult/minor mismatch, not-applicable guardian state, or ordinary eligibility |
| External extraction ignores access, terms, attribution, quotation, privacy, or deletion | A content-free boundary records each dimension independently and permits only active public, terms-allowed, attributed, privacy-passing synthetic sources; raw source body is always false | Any private/member/unknown/deleted, terms-unresolved, attribution/quotation, privacy, or raw-body state fails closed |
| Deleted source silently erases provenance or is retained without independent consent | Retention binds the deleted event/version and same source aggregate to the exact lead/public version, preserves content-free provenance, and requires independent reporter lead consent plus privacy policy | Body retention, provenance erasure, unrelated source, withdrawn consent, or failed/held policy rejects retention |
| Payment or private intake is treated as publication permission | A separate paid-private boundary fixes private visibility, no forum record, no public projection, no raw intake body, and a later separate public-lead workflow | Contract/database reject initial public consent or any immediate forum/public projection |
