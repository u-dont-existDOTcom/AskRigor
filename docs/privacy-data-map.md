...
| Disabled Phase I Hermes semantic worker | One opaque session ID and state digest; the exact current routing or public-candidate work package; for new-session module routing, the same deterministically screened de-identified population-level target accepted by the private start route; one dedicated model credential; provider/model and non-authoritative API-call/cost diagnostics | One fresh child process and temporary directory per work package. Hermes tools, memory, context files, trajectories, checkpoints, and background review are disabled. The temporary directory is removed after the turn; AskRigor writes no Hermes conversation or model output to a new store. The external model provider may process the transient package under its own terms. The child never receives the private orchestration credential, production research-provider secrets, raw private health content, raw public-source corpora, or a finalization permit. Phase I adds no deployment or live recipient by default. |
