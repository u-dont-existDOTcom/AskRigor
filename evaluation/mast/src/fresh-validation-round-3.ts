...
import { aggregateFreshValidation } from "./fresh-validation-round-3-base.js";
import {
...
export const ROUND_3_STUDY_ID = "askrigor-mast-fresh-validation-round-3-20260919";
export const ROUND_3_ARMS = ["A", "B", "C", "D"] as const;
export const ROUND_3_JUDGES = ["J1", "J2", "J3"] as const;
export const ROUND_3_GENERATION_COUNT = 120;
export const ROUND_3_PRIMARY_JUDGMENT_COUNT = 240;
...
export function deriveRound3Selection(input: {
  ...
  if (new Set(allFamilies).size !== allFamilies.length || allFamilies.some((id) => !familySchema.safeParse(id).success)) {
    throw new Error("ROUND_3_IDENTIFIER_MANIFEST_INVALID");
  }
  ...
  if (eligibleFamilies.length !== 11) throw new Error("ROUND_3_ELIGIBLE_FAMILY_COUNT_INVALID");
  const reserveFamilies = ["All008"];
  if (!eligibleFamilies.includes(reserveFamilies[0]!)) throw new Error("ROUND_3_RESERVE_MISSING");
  ...
  if (validationFamilies.length !== 10
    ...
    throw new Error("ROUND_3_VALIDATION_SELECTION_INVALID");
  }
...
export function detectPrimaryDisagreements(j1Input: unknown[], j2Input: unknown[]): Array<{
  ...
  if (new Set(j1.map(({ opaqueResponseId }) => opaqueResponseId)).size !== j1.length
    || byJ2.size !== j2.length) throw new Error("ROUND_3_PRIMARY_JUDGE_ID_DUPLICATE");
  const disagreements = [];
  for (const left of j1) {
    const right = byJ2.get(left.opaqueResponseId);
    if (!right) throw new Error("ROUND_3_PRIMARY_JUDGE_COVERAGE_INVALID");
    if (left.exactGenerationOutputSha256 !== right.exactGenerationOutputSha256
      || left.exactPacketSha256 !== right.exactPacketSha256) {
      throw new Error("ROUND_3_PRIMARY_JUDGE_SOURCE_MISMATCH");
    }
  ...
  if (j1.length !== j2.length) throw new Error("ROUND_3_PRIMARY_JUDGE_COVERAGE_INVALID");
...
export function reconcileJudgments(input: {
  ...
  if (j3ById.size !== j3.length || j3.length !== disagreements.length
    ...
    throw new Error("ROUND_3_ADJUDICATION_COVERAGE_INVALID");
  }
...
export function expectedBenchmarkConflictReviews(input: {
  ...
  if (byOpaque.size !== input.mapping.length) throw new Error("ROUND_3_CONFLICT_REVIEW_MAPPING_DUPLICATE");
  const keys = new Map<string, { familyId: string; actionId: string }>();
  for (const target of input.targets) {
    const mapped = byOpaque.get(target.opaqueResponseId);
    if (!mapped) throw new Error("ROUND_3_CONFLICT_REVIEW_MAPPING_ORPHAN");
    if (target.actionIds.length === 0 || new Set(target.actionIds).size !== target.actionIds.length) {
      throw new Error("ROUND_3_CONFLICT_REVIEW_ACTION_INVALID");
    }
...
export function scoreRound3(input: {
  generation: GenerationIdentityRecord[];
  mapping: ConditionMapRecord[];
...
export function makeFailureReceipt(input: Omit<FailureReceipt, "schemaVersion" | "studyId">): FailureReceipt {
  return failureReceiptSchema.parse({ schemaVersion: 1, studyId: ROUND_3_STUDY_ID, ...input });
}
...
export function verifyCapturedBytes(bytes: Uint8Array, expectedSha256: string, expectedLength: number): void {
  if (bytes.byteLength !== expectedLength || sha256(bytes) !== expectedSha256) {
    throw new Error("ROUND_3_CAPTURE_BYTES_MISMATCH");
  }
}

export function buildMachineSummary(input: {
  result: ReturnType<typeof scoreRound3>;
