...
export const ROUND_2_STUDY_ID = "askrigor-mast-fresh-validation-round-2-20260918";
export const ROUND_2_ARMS = ["A", "B", "C", "D"] as const;
export const ROUND_2_JUDGES = ["J1", "J2", "J3"] as const;
export const ROUND_2_GENERATION_COUNT = 144;
export const ROUND_2_PRIMARY_JUDGMENT_COUNT = 288;
...
export function deriveRound2Selection(input: {
  allFamilies: string[];
  developmentFamilies: string[];
  generatedRound1Families: string[];
  seedLabel: string;
}): { eligibleFamilies: string[]; validationFamilies: string[]; confirmationFamilies: string[] } {
  ...
  if (new Set(allFamilies).size !== allFamilies.length || allFamilies.some((id) => !familySchema.safeParse(id).success)) {
    throw new Error("ROUND_2_IDENTIFIER_MANIFEST_INVALID");
  }
  const excluded = new Set([...input.developmentFamilies, ...input.generatedRound1Families]);
  const eligibleFamilies = allFamilies.filter((id) => !excluded.has(id));
  if (eligibleFamilies.length !== 13) throw new Error("ROUND_2_ELIGIBLE_FAMILY_COUNT_INVALID");
  const counts = new Map<string, number>();
  for (const family of eligibleFamilies) counts.set(familyPrefix(family), (counts.get(familyPrefix(family)) ?? 0) + 1);
  if (counts.size !== 10) throw new Error("ROUND_2_SPECIALTY_COVERAGE_INVALID");
  const reserveCandidates = eligibleFamilies.filter((id) => (counts.get(familyPrefix(id)) ?? 0) > 1);
  reserveCandidates.sort((left, right) => createHash("sha256")
    .update(input.seedLabel).update("\0").update(left).digest("hex").localeCompare(createHash("sha256")
      .update(input.seedLabel).update("\0").update(right).digest("hex")));
  const confirmationFamilies = [reserveCandidates[0]!];
  const validationFamilies = eligibleFamilies.filter((id) => id !== confirmationFamilies[0]);
  if (validationFamilies.length !== 12
    || new Set(validationFamilies.map(familyPrefix)).size !== 10) {
    throw new Error("ROUND_2_VALIDATION_SELECTION_INVALID");
  }
  return { eligibleFamilies, validationFamilies, confirmationFamilies };
}
...
export function createRound2Dispatch(seed: Uint8Array, validationFamilies: string[]) {
...
export function detectPrimaryDisagreements(j1Input: unknown[], j2Input: unknown[]): Array<{
  ...
  if (new Set(j1.map(({ opaqueResponseId }) => opaqueResponseId)).size !== j1.length
    || byJ2.size !== j2.length) throw new Error("ROUND_2_PRIMARY_JUDGE_ID_DUPLICATE");
  const disagreements = [];
  for (const left of j1) {
    const right = byJ2.get(left.opaqueResponseId);
    if (!right) throw new Error("ROUND_2_PRIMARY_JUDGE_COVERAGE_INVALID");
    if (left.exactGenerationOutputSha256 !== right.exactGenerationOutputSha256
      || left.exactPacketSha256 !== right.exactPacketSha256) {
      throw new Error("ROUND_2_PRIMARY_JUDGE_SOURCE_MISMATCH");
    }
  ...
  if (j1.length !== j2.length) throw new Error("ROUND_2_PRIMARY_JUDGE_COVERAGE_INVALID");
...
export function reconcileJudgments(input: {
  ...
  if (j3ById.size !== j3.length || j3.length !== disagreements.length
    ...
    throw new Error("ROUND_2_ADJUDICATION_COVERAGE_INVALID");
  }
...
export function scoreRound2(input: {
  generation: GenerationIdentityRecord[];
  mapping: ConditionMapRecord[];
  ...
  if (joined.integrityErrors.length > 0) {
    return { ...aggregateFreshValidation({ records: [], expectedFamilies: input.expectedFamilies }),
      integrityErrors: joined.integrityErrors };
  }
  return aggregateFreshValidation({ records: joined.records, expectedFamilies: input.expectedFamilies });
}

export function makeFailureReceipt(input: Omit<FailureReceipt, "schemaVersion" | "studyId">): FailureReceipt {
  return failureReceiptSchema.parse({ schemaVersion: 1, studyId: ROUND_2_STUDY_ID, ...input });
}
...
export function verifyCapturedBytes(bytes: Uint8Array, expectedSha256: string, expectedLength: number): void {
  if (bytes.byteLength !== expectedLength || sha256(bytes) !== expectedSha256) {
    throw new Error("ROUND_2_CAPTURE_BYTES_MISMATCH");
  }
}

export function buildMachineSummary(input: {
  result: ReturnType<typeof scoreRound2>;
