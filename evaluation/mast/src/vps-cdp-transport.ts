...
import { ROUND_2_GENERATION_COUNT, ROUND_2_STUDY_ID } from "./fresh-validation-round-2.js";
...
export const VPS_CDP_ENDPOINT = "http://127.0.0.1:9222" as const;
export const VPS_BROWSER = "Brave" as const;
export const VPS_PRIVATE_ROOT = "/home/cloudbrowser/.local/share/askrigor-mast-round2" as const;

export const vpsPacketTransferRecordSchema = z.object({
  sequence: z.number().int().min(1).max(ROUND_2_GENERATION_COUNT),
...
export const vpsPacketTransferReceiptSchema = z.object({
  ...
  records: z.array(vpsPacketTransferRecordSchema).length(ROUND_2_GENERATION_COUNT),
}).strict().superRefine((receipt, context) => {
  if (new Set(receipt.records.map(({ sequence }) => sequence)).size !== ROUND_2_GENERATION_COUNT
    || new Set(receipt.records.map(({ opaqueInputId }) => opaqueInputId)).size !== ROUND_2_GENERATION_COUNT
