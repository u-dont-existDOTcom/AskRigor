...
import { ROUND_3_GENERATION_COUNT, ROUND_3_STUDY_ID } from "./fresh-validation-round-3.js";
...
export const VPS_PRIVATE_ROOT = "/home/cloudbrowser/.local/share/askrigor-mast-round3" as const;

export const vpsPacketTransferRecordSchema = z.object({
  sequence: z.number().int().min(1).max(ROUND_3_GENERATION_COUNT),
...
export const vpsPacketTransferReceiptSchema = z.object({
  ...
  records: z.array(vpsPacketTransferRecordSchema).length(ROUND_3_GENERATION_COUNT),
}).strict().superRefine((receipt, context) => {
  if (new Set(receipt.records.map(({ sequence }) => sequence)).size !== ROUND_3_GENERATION_COUNT
    || new Set(receipt.records.map(({ opaqueInputId }) => opaqueInputId)).size !== ROUND_3_GENERATION_COUNT
