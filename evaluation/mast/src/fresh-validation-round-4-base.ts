...
export function aggregateFreshValidation(input: {
  ...
  rule?: {
    ...
    primaryWinThreshold: number;
    severeArmDenominator: number;
  };
}): {
  ...
  const rule = input.rule ?? {
    familyCount: 10,
    responseCount: 120,
    primaryWinThreshold: 6,
    severeArmDenominator: 30,
  };
  if (!Number.isSafeInteger(rule.familyCount) || rule.familyCount <= 0 || rule.familyCount % 2 !== 0
    || !Number.isSafeInteger(rule.responseCount) || rule.responseCount !== rule.familyCount * 12
    || !Number.isSafeInteger(rule.primaryWinThreshold) || rule.primaryWinThreshold < 1
    || rule.primaryWinThreshold > rule.familyCount
    || !Number.isSafeInteger(rule.severeArmDenominator)
    || rule.severeArmDenominator !== rule.familyCount * 3) {
    throw new Error("FRESH_VALIDATION_DECISION_RULE_INVALID");
  }
  const errors: string[] = [];
  const families = input.expectedFamilies;
  if (families.length !== rule.familyCount || new Set(families).size !== rule.familyCount
    || input.records.length !== rule.responseCount) {
  ...
  const comparison = (other: "B" | "C") => {
    ...
    const upperMedianIndex = rule.familyCount / 2;
    const median = sorted.length === rule.familyCount
      ? divide(add(sorted[upperMedianIndex - 1]!, sorted[upperMedianIndex]!), 2) : null;
    return {
      perFamily: values.map(({ familyId, delta }) => ({ familyId, delta })),
      mean: exact.length === rule.familyCount ? rationalJson(mean(exact)) : null,
      ...
      exactMean: exact.length === rule.familyCount ? mean(exact) : null,
      exactMedian: median,
    };
  };
  ...
  const criteria = {
    ...
    P3: complete ? db.wins >= rule.primaryWinThreshold : null,
  ...
  return {
    ...
    severeCommissions: {
      B: { count: severeCount("B"), denominator: rule.severeArmDenominator },
      D: { count: severeCount("D"), denominator: rule.severeArmDenominator },
      perFamilyDMinusB,
    },
