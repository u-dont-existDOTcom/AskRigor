...
import { ROUND_4_STUDY_ID } from "./fresh-validation-round-4.js";

const digestSchema = z.string().regex(/^[0-9a-f]{64}$/u);
const instantSchema = z.string().datetime({ offset: true });
const runIdSchema = z.string().regex(/^(?:run-[0-9a-f]{24}|synthetic-normal-path)$/u);

// Round 4 intentionally reuses the already-qualified Round-3 browser runtime.
// The study workspace and receipts are new; the supervised browser bytes are not.
export const ROUND_4_BROWSER_PROFILE = "/home/cloudbrowser/.config/brave-mast-round3" as const;
export const ROUND_4_BROWSER_SERVICE = "askrigor-mast-round3-brave.service" as const;
export const ROUND_4_DISPLAY_SERVICE = "askrigor-mast-round3-xvfb.service" as const;
export const ROUND_4_WATCHDOG_SERVICE = "askrigor-mast-round3-watchdog.service" as const;
export const ROUND_4_CDP_ENDPOINT = "http://127.0.0.1:9224" as const;
...
export function verifyRuntimeFileHashes(expected: unknown, observed: unknown): RuntimeFileHashes {
  ...
  for (const key of Object.keys(left) as Array<keyof RuntimeFileHashes>) {
    if (left[key] !== right[key]) throw new Error(`ROUND_4_RUNTIME_HASH_DRIFT:${key}`);
  }
  return right;
}
...
export const sentReceiptSchema = z.object({
  ...
  browserProfile: z.literal(ROUND_4_BROWSER_PROFILE),
  browserService: z.literal(ROUND_4_BROWSER_SERVICE),
  displayService: z.literal(ROUND_4_DISPLAY_SERVICE),
  watchdogService: z.literal(ROUND_4_WATCHDOG_SERVICE),
  cdpEndpoint: z.literal(ROUND_4_CDP_ENDPOINT),
  ...
  if (receipt.sourceSha256 !== receipt.destinationPacketSha256
    ...
    context.addIssue({ code: "custom", message: "ROUND_4_SENT_IDENTITY_MISMATCH" });
  }
});
...
export function selectExactRecoveryCandidate(input: {
  ...
  if (!Number.isSafeInteger(input.expectedUtf8Bytes) || input.expectedUtf8Bytes <= 0) {
    throw new Error("ROUND_4_RECOVERY_EXPECTED_LENGTH_INVALID");
  }
  ...
  if (matching.length === 0) throw new Error("ROUND_4_RECOVERY_CANDIDATE_ZERO");
