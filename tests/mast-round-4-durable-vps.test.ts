...
import {
  ...
  ROUND_4_BROWSER_PROFILE,
  ROUND_4_BROWSER_SERVICE,
  ROUND_4_CDP_ENDPOINT,
  ROUND_4_DISPLAY_SERVICE,
  ROUND_4_WATCHDOG_SERVICE,
  selectExactRecoveryCandidate,
  sentReceiptSchema,
  verifyRuntimeFileHashes,
} from "../evaluation/mast/src/durable-vps-round-4.js";
import {
  ...
  "../evaluation/mast/src/chatgpt-browser-transport-round-4.js";
import { ROUND_4_STUDY_ID } from "../evaluation/mast/src/fresh-validation-round-4.js";
import { vpsPacketTransferReceiptSchema } from
  "../evaluation/mast/src/vps-cdp-transport-round-4.js";
...
describe("Round 4 durable VPS runtime and recovery", () => {
  it("detects every frozen runtime file tamper", () => {
    ...
    expect(() => verifyRuntimeFileHashes(hashSet, { ...hashSet,
      browserWrapperSha256: "f".repeat(64) })).toThrow("ROUND_4_RUNTIME_HASH_DRIFT");
  });
  ...
  it("fails closed for zero or multiple exact recovery candidates", () => {
    ...
    expect(() => selectExactRecoveryCandidate({ ...expected, expectedSha256: expected.sha256,
      expectedUtf8Bytes: expected.utf8Bytes, candidates: [] })).toThrow("ROUND_4_RECOVERY_CANDIDATE_ZERO");
    expect(() => selectExactRecoveryCandidate({ expectedSha256: expected.sha256,
      expectedUtf8Bytes: expected.utf8Bytes,
      candidates: [candidate("one", "same"), candidate("two", "same")] }))
      .toThrow("ROUND_4_RECOVERY_CANDIDATE_MULTIPLE");
  });
  ...
  it("accepts a real-size exact composer projection and rejects truncation or one altered character", () => {
    const source = `${"round4-exactness-αβγ".repeat(9_000)}\nterminal`;
  ...
  it("requires source, destination, and composer equality in the SENT receipt", () => {
    ...
    const base = {
      ...
      browserProfile: ROUND_4_BROWSER_PROFILE,
      browserService: ROUND_4_BROWSER_SERVICE,
      displayService: ROUND_4_DISPLAY_SERVICE,
      watchdogService: ROUND_4_WATCHDOG_SERVICE,
      cdpEndpoint: ROUND_4_CDP_ENDPOINT,
      sentAt: instant,
    };
    ...
    expect(() => sentReceiptSchema.parse({ ...base, composerSha256: digest("altered") }))
      .toThrow("ROUND_4_SENT_IDENTITY_MISMATCH");
  });

  it("rejects wrong model, effort, session, personalization, and transport provenance", () => {
    const source = normalizedTextIdentity("packet");
    const runtime = {
      schemaVersion: 1 as const, studyId: ROUND_4_STUDY_ID,
      browserProfile: ROUND_4_BROWSER_PROFILE, browserService: ROUND_4_BROWSER_SERVICE,
      displayService: ROUND_4_DISPLAY_SERVICE, watchdogService: ROUND_4_WATCHDOG_SERVICE,
      cdpEndpoint: ROUND_4_CDP_ENDPOINT, browserMainPid: 123, observed: hashSet, observedAt: instant,
    };
    const receipt: any = {
      schemaVersion: 1, studyId: ROUND_4_STUDY_ID,
      ...
      vpsDevice: "srv1894948", vpsUser: "cloudbrowser", cdpEndpoint: ROUND_4_CDP_ENDPOINT,
      cdpAttached: true, browser: "Brave", browserProfile: ROUND_4_BROWSER_PROFILE,
      browserService: ROUND_4_BROWSER_SERVICE, displayService: ROUND_4_DISPLAY_SERVICE,
      watchdogService: ROUND_4_WATCHDOG_SERVICE, runtimeAttestation: runtime, tabCount: 1,
  ...
  it("requires exact 120-packet transfer coverage and rejects a destination mismatch", () => {
    ...
    const receipt = { schemaVersion: 1, studyId: ROUND_4_STUDY_ID, device: "srv1894948",
      user: "cloudbrowser", privateRoot: "/home/cloudbrowser/.local/share/askrigor-mast-round4",
  ...
  it("freezes the supervised single-tab direct-DOM architecture with no relay, clipboard, or typing", () => {
    const transport = readFileSync(resolve(root, "scripts/mast-vps-cdp-generation-round-4.mjs"), "utf8");
    ...
    const orchestrator = readFileSync(resolve(root, "scripts/mast-vps-round4-orchestrator.mts"), "utf8");
    ...
    expect(transport).toContain("ROUND_4_JUDGMENT_TRANSPORT_ATTEMPT_CEILING_EXHAUSTED");
