...
const manifest = JSON.parse(readFileSync(resolve(root,
  "evaluation/mast/fresh-validation-round-3-family-manifest.json"), "utf8"));
...
describe("MAST Fresh Validation Round 3 cohort and frozen rule", () => {
  ...
  it("creates 120 deterministic unique opaque slots", () => {
    ...
    expect(first).toHaveLength(ROUND_3_GENERATION_COUNT);
  ...
  it("uses the 5th/6th median, six-win threshold, and equal 30-response severe denominators", () => {
    ...
    const result = scoreRound3({ ...scoredFixture({ dScores, bScores: baseScores, cScores: baseScores }),
...
describe("MAST Fresh Validation Round 3 schema, blinding, and completion boundaries", () => {
  it("binds every executable, the exact owner directive, and the Round-2 historical disposition", () => {
    const environment = JSON.parse(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-3-environment.json"), "utf8"));
    const preregistration = JSON.parse(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-3-preregistration.json"), "utf8"));
    ...
    expect(createHash("sha256").update(readFileSync(resolve(root,
      "docs/directives/2026-09-19-mast-fresh-validation-round-3-durable-vps.md"))).digest("hex"))
      .toBe("16c302bc3b0646f6b86597ba53191dd1d5c37a3c98a6ff996a03d1c1f67c41d6");
    expect(createHash("sha256").update(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-3-environment.json"))).digest("hex"))
      .toBe(preregistration.bindings.environmentSha256);
    expect(createHash("sha256").update(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-3-family-manifest.json"))).digest("hex"))
  ...
  it("requires complete primary and triggered adjudication coverage", () => {
    ...
    expect(() => reconcileJudgments({ generation: [], mapping: [], j1: [j1], j2: [j2], j3: [] }))
      .toThrow("ROUND_3_ADJUDICATION_COVERAGE_INVALID");
    expect(() => detectPrimaryDisagreements([j1], [])).toThrow("ROUND_3_PRIMARY_JUDGE_COVERAGE_INVALID");
  });

  it("binds blinded schedules to the exact generation output hash used by capture", () => {
    const source = readFileSync(resolve(root, "scripts/mast-fresh-validation-round-3.mts"), "utf8");
  ...
  it("makes incomplete coverage INDETERMINATE and validation-ineligible", () => {
    const result = scoreRound3({ generation: [], mapping: [], final: [],
      expectedFamilies: manifest.validationFamilies });
