...
import { admitGenerationCapture } from "../scripts/mast-fresh-validation-round-5.mts";

const root = resolve(import.meta.dirname, "..");
const manifest = JSON.parse(readFileSync(resolve(root,
  "evaluation/mast/fresh-validation-round-5-family-manifest.json"), "utf8"));
...
describe("MAST Fresh Validation Round 5 cohort and frozen rule", () => {
  it("selects the exact ten families, consumes All008, and excludes Round-4-exposed Card002", () => {
    expect(deriveRound5Selection({
      ...
      exposedRound4Families: manifest.excludedRound4ExposedFamilies,
    })).toEqual({
      eligibleFamilies: manifest.eligibleFamilies,
      validationFamilies: manifest.validationFamilies,
      reserveFamilies: [],
    });
    expect(manifest.reserveStatus.status).toBe("NO_REMAINING_SEMANTIC_RESERVE");
    expect(manifest.validationFamilies).toContain("All008");
    expect(manifest.validationFamilies).not.toContain("Card002");
  ...
  it("creates 120 deterministic unique opaque slots", () => {
    ...
    const first = createRound5Dispatch(seed, manifest.validationFamilies);
    expect(first).toEqual(createRound5Dispatch(seed, manifest.validationFamilies));
    expect(first).toHaveLength(ROUND_5_GENERATION_COUNT);
  ...
  it("uses the 5th/6th median, six-win threshold, and equal 30-response severe denominators", () => {
    ...
    const result = scoreRound5({ ...scoredFixture({ dScores, bScores: baseScores, cScores: baseScores }),
...
describe("MAST Fresh Validation Round 5 schema, blinding, and completion boundaries", () => {
  it("binds every executable, the exact owner directive, and the Round-2 historical disposition", () => {
    ...
    expect(createHash("sha256").update(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-5-environment.json"))).digest("hex"))
      .toBe(preregistration.bindings.environmentSha256);
    expect(createHash("sha256").update(readFileSync(resolve(root,
      "evaluation/mast/fresh-validation-round-5-family-manifest.json"))).digest("hex"))
  ...
  it("admits required chatMode through the production generation-capture admission path", () => {
    const opaqueInputId = "run-000000000000000000000001";
    const packet = "synthetic round-5 capture packet";
    const output = Buffer.from("synthetic provider response", "utf8");
    const exactInputSha256 = digest(packet);
    const transport = {
      schemaVersion: 1,
      studyId: ROUND_5_STUDY_ID,
      opaqueInputId,
      ...
      sourceUtf8Bytes: Buffer.byteLength(packet),
      sourceCodePoints: [...packet].length,
      sourceSha256: exactInputSha256,
      destinationPacketUtf8Bytes: Buffer.byteLength(packet),
      destinationPacketCodePoints: [...packet].length,
      destinationPacketSha256: exactInputSha256,
      composerUtf8Bytes: Buffer.byteLength(packet),
      composerCodePoints: [...packet].length,
      composerSha256: exactInputSha256,
      exactEquality: true,
      ui: {
        schemaVersion: 1, observedAt: instant, origin: "https://chatgpt.com",
        modelVisibleLabel: "GPT-5.6 Sol", reasoningVisibleLabel: "Extra High",
        reasoningOrdinal: "4 of 5", chatMode: "TEMPORARY", personalization: "UNPERSONALIZED",
        authenticated: true, freshConversation: true, userMessageCount: 0, assistantMessageCount: 0,
        attachmentCount: 0, sendEnabled: true,
      },
      vpsDevice: "srv1894948", vpsUser: "cloudbrowser", cdpEndpoint: "http://127.0.0.1:9224",
      cdpAttached: true, browser: "Brave", browserProfile: "/home/cloudbrowser/.config/brave-mast-round3",
      browserService: "askrigor-mast-round3-brave.service", displayService: "askrigor-mast-round3-xvfb.service",
      watchdogService: "askrigor-mast-round3-watchdog.service",
      runtimeAttestation: {
        schemaVersion: 1, studyId: ROUND_5_STUDY_ID, browserProfile: "/home/cloudbrowser/.config/brave-mast-round3",
        browserService: "askrigor-mast-round3-brave.service", displayService: "askrigor-mast-round3-xvfb.service",
        watchdogService: "askrigor-mast-round3-watchdog.service", cdpEndpoint: "http://127.0.0.1:9224",
        browserMainPid: 1234, observed: {
          browserUnitSha256: digest("browser-unit"), displayUnitSha256: digest("display-unit"),
          watchdogUnitSha256: digest("watchdog-unit"), browserWrapperSha256: digest("browser-wrapper"),
          watchdogScriptSha256: digest("watchdog-script"), transportScriptSha256: digest("transport-script"),
        }, observedAt: instant,
      },
      tabCount: 1, citationUrls: [], toolProvenance: [], responseArtifactSha256: digest(output.toString("utf8")),
      verifiedAt: instant, sentAt: instant, completedAt: instant, recoveredExistingSubmission: false,
    };
    const providerValue = { ...provider("GENERATION"), chatMode: "TEMPORARY" };
    const capture = admitGenerationCapture({
      opaqueInputId, exactInputSha256, output, providerValue,
      transportBytes: Buffer.from(JSON.stringify(transport), "utf8"),
    });
    expect(capture.provider.chatMode).toBe("TEMPORARY");
    expect(capture.exactOutputSha256).toBe(digest(output.toString("utf8")));
    const { chatMode: _removed, ...missingChatMode } = providerValue;
    expect(() => admitGenerationCapture({
      opaqueInputId, exactInputSha256, output, providerValue: missingChatMode,
      transportBytes: Buffer.from(JSON.stringify(transport), "utf8"),
    })).toThrow();
  });
  ...
  it("requires complete primary and triggered adjudication coverage", () => {
    ...
    expect(() => reconcileJudgments({ generation: [], mapping: [], j1: [j1], j2: [j2], j3: [] }))
      .toThrow("ROUND_5_ADJUDICATION_COVERAGE_INVALID");
    expect(() => detectPrimaryDisagreements([j1], [])).toThrow("ROUND_5_PRIMARY_JUDGE_COVERAGE_INVALID");
  });

  it("binds blinded schedules to the exact generation output hash used by capture", () => {
    const source = readFileSync(resolve(root, "scripts/mast-fresh-validation-round-5.mts"), "utf8");
  ...
  it("makes incomplete coverage INDETERMINATE and validation-ineligible", () => {
    const result = scoreRound5({ generation: [], mapping: [], final: [],
      expectedFamilies: manifest.validationFamilies });
