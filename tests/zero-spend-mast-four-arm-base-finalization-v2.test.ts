...
describe("zero-spend blinded MAST evaluator v2 finalization", () => {
  ...
  it("accepts one owner-authorized replacement tab only after a transport failure", () => {
    const slot = {
      j3Ordinal: 1,
      opaqueResponseId: "EVAL-0123456789abcdef01234567",
      caseId: "Derm001",
    ...
    const failure = {
      ...
      modelSlugObserved: null,
      capturedAt: "2026-09-05T05:00:00Z",
      chatLocator: "https://chatgpt.com/c/failure-1",
      conversationId: "failure-1",
      ...
      ...latestReceiptProvenance({
        ordinal: 1,
        opaqueResponseId: slot.opaqueResponseId,
        packetSha256: slot.exactPacketSha256,
        attempt: 1,
        capturedAt: "2026-09-05T05:00:00Z",
      }),
    } as const;
    const valid = {
      ...
      thinkingEffortObserved: "TEST_MAXIMUM_UI_LABEL",
      modelSlugObserved: null,
      chatLocator: "https://chatgpt.com/c/valid-2",
      conversationId: "valid-2",
      userMessageId: "user-2",
      assistantMessageId: "assistant-2",
      sentAtSource: null,
      sentAtSourceStatus: "UNAVAILABLE",
      capturedAt: "2026-09-05T05:10:00Z",
      toolsInvoked: false,
      browsingInvoked: false,
      manualToolSelection: false,
      automaticToolInvocationObserved: false,
      ...
      provenanceStatus: "VERIFIED",
      transport: "PASTED_TEXT_ATTACHMENT",
      outputFile: "evaluation-v2/j3-latest-restart/judgments/test.txt",
      exactOutputSha256: "7".repeat(64),
      ...
      physical_tab_id: "663931999",
      physical_tab_transition: {
    ...
    const progress = {
      ...
      sourceJ3ScheduleSha256: sha256(`${JSON.stringify(schedule, null, 2)}\n`),
      j3JudgmentTarget: 1,
      validJudgmentCount: 1,
      mechanicalFailureCount: 1,
      records: [valid],
      mechanicalFailures: [failure],
      haltedClaim: null,
    } as const;
    expect(acceptJ3CaptureProgress(schedule, progress, primary).records).toHaveLength(1);
    expect(() => acceptJ3CaptureProgress(schedule, {
      ...progress,
      records: [{ ...valid, physical_tab_transition: undefined }],
    }, primary)).toThrow("EVALUATOR_V2_J3_PHYSICAL_TAB_REPLACEMENT_INVALID");
  });
});
