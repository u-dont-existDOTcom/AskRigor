...
async function verifyRuntime(expectedPath) {
  ...
  for (const key of Object.keys(observed)) {
    if (!/^[0-9a-f]{64}$/u.test(expected[key] ?? "") || observed[key] !== expected[key]) {
      throw new Error(`ROUND_5_RUNTIME_HASH_DRIFT_${key.toUpperCase()}`);
    }
  }
  ...
  if (browser.ActiveState !== "active" || browser.SubState !== "running"
    ...
    throw new Error("ROUND_5_RUNTIME_SERVICE_NOT_RUNNING");
  }
  const pid = Number(browser.MainPID);
  if (!Number.isSafeInteger(pid) || pid <= 1) throw new Error("ROUND_5_BROWSER_MAIN_PID_INVALID");
...
function validateJudgeManifest(manifest) {
  ...
  for (const record of manifest.records) {
    if (!/^EVAL-[0-9a-f]{24}$/u.test(record.opaqueResponseId)
      ...
      throw new Error("ROUND_5_JUDGE_MANIFEST_RECORD_INVALID");
    }
    ids.add(record.opaqueResponseId);
  }
  if (ids.size !== manifest.records.length) throw new Error("ROUND_5_JUDGE_MANIFEST_DUPLICATE");
...
async function verifyJudgeTransfer() {
  ...
  for (const source of manifest.records) {
    ...
    if (identity.sha256 !== source.expectedSha256 || identity.utf8Bytes !== source.sourceUtf8Bytes) {
      throw new Error(`ROUND_5_JUDGE_PACKET_DESTINATION_MISMATCH_${source.opaqueResponseId}`);
    }
  ...
  process.stdout.write(`${JSON.stringify({ status: "ROUND_5_JUDGE_PACKET_TRANSFER_VERIFIED", judge: config.judge, packetCount: records.length, receiptSha256: sha256(Buffer.from(canonicalJson(receipt), "utf8")) })}\n`);
  process.exit(0);
}
...
async function establishFreshTemporaryUnpersonalized(page) {
  ...
  const temporaryDeadline = Date.now() + 30_000;
  ...
  if (temporaryState === "OFF") {
    ...
    await waitForExactButton(page, "Turn off temporary chat", 30_000);
...
async function waitForSubmittedRequestIdentity(page, source) {
  ...
  throw new Error("ROUND_5_SUBMITTED_CONVERSATION_IDENTITY_NOT_OBSERVED");
}
...
async function exactRecoveryCandidate(source, preferredConversationUrl = null, expectedRequestMessageId = null) {
  ...
  if (preferredConversationUrl !== null && exactPreferredUrl === null) {
    throw new Error("ROUND_5_RECOVERY_CONVERSATION_URL_INVALID");
  }
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (safeFailureCode(error) === "ROUND_5_RECOVERY_CANDIDATE_MULTIPLE") throw error;
    }
    await sleep(1_000);
  }
  throw new Error(lastCandidateCount === 0
    ? "ROUND_5_RECOVERY_CANDIDATE_ZERO" : "ROUND_5_RECOVERY_CANDIDATE_UNRESOLVED");
}
...
async function recoverExistingSubmission({ workspace, record, attempt }) {
  ...
  const sentIdentityPath = join(runDirectory, `attempt-${attempt}-sent-identity.json`);
...
async function runAttempt({ workspace, record, attempt, runtimeAttestation, expectedRuntimeHashes }) {
  const runDirectory = join(workspace, "runs", record.opaqueInputId);
...
async function runOne() {
  ...
  if (await exists(runDirectory)) {
    ...
    if (submissionAttempts.length > 1) throw new Error("ROUND_5_DUPLICATE_SEND_STATE_DETECTED");
  ...
  while (prior < MAXIMUM_ATTEMPTS) {
    ...
    try {
      const result = await runAttempt({ workspace, record, attempt, runtimeAttestation, expectedRuntimeHashes });
      process.stdout.write(`${JSON.stringify(result)}\n`);
      process.exit(0);
    } catch (error) {
...
async function runJudgeAttempt({ workspace, record, attempt, runtimeAttestation, expectedRuntimeHashes, config }) {
  ...
  try {
    ...
    if (source.sha256 !== record.expectedSha256 || source.utf8Bytes !== record.sourceUtf8Bytes) {
      throw new Error("ROUND_5_JUDGE_PACKET_DESTINATION_MISMATCH");
    }
    stage = "CDP_ATTACH";
    const browser = await connect();
    const { page, tabCount } = await ensureChatGptPage(browser.contexts());
...
async function runJudgeOne() {
  ...
  if (!record) throw new Error("ROUND_5_JUDGMENT_SLOT_UNKNOWN");
  ...
  while (prior < MAXIMUM_JUDGE_ATTEMPTS) {
    const attempt = prior + 1;
    try {
      const result = await runJudgeAttempt({ workspace, record, attempt, runtimeAttestation, expectedRuntimeHashes, config });
      process.stdout.write(`${JSON.stringify(result)}\n`);
      process.exit(0);
    } catch (error) {
      if (await exists(join(runDirectory, "ambiguity-receipt.json"))) throw error;
  ...
  throw new Error("ROUND_5_JUDGMENT_TRANSPORT_ATTEMPT_CEILING_EXHAUSTED");
}
...
async function markJudgeSealed() {
  ...
  if (!/^EVAL-[0-9a-f]{24}$/u.test(opaqueResponseId) || !/^[0-9a-f]{64}$/u.test(captureSha256)) {
    throw new Error("ROUND_5_JUDGMENT_SEAL_ARGUMENT_INVALID");
  }
  const runDirectory = join(workspace, "runs", opaqueResponseId);
  ...
  if (ready.responseSha256 !== sha256(response)) throw new Error("ROUND_5_JUDGMENT_SEAL_RESPONSE_MISMATCH");
...
async function runtimeVerification() {
  ...
  process.stdout.write(canonicalJson({ status: "ROUND_5_RUNTIME_VERIFIED", ...receipt }));
  process.exit(0);
}

async function deterministicSupervisionAcceptance() {
  const expectedRuntimeHashes = resolve(requiredArgument("--expected-runtime-hashes"));
  const runtime = await verifyRuntime(expectedRuntimeHashes);
  const first = await connect();
  const firstContexts = first.contexts().length;
  await first.close();
  const second = await connect();
  const selected = await ensureChatGptPage(second.contexts());
  const state = await safePageState(selected.page);
  await second.close();
  if (firstContexts < 1 || state.origin !== CHATGPT_ORIGIN) throw new Error("CDP_RECONNECT_PRE_SEND_FAILED");
  process.stdout.write(canonicalJson({
    schemaVersion: 1,
    studyId: STUDY_ID,
    status: "DETERMINISTIC_SUPERVISION_PASS",
    sent: false,
    cdpDisconnectReconnectBeforeSend: true,
...
async function syntheticNormalPathAcceptance() {
  const workspace = resolve(requiredArgument("--workspace"));
  const expectedRuntimeHashes = resolve(requiredArgument("--expected-runtime-hashes"));
  const runDirectory = join(workspace, "acceptance", "synthetic-normal-path");
  ...
  const prompt = "Reply exactly ROUND4_OK.";
  const source = textIdentity(prompt);
  const record = { opaqueInputId: "synthetic-normal-path", sequence: 0 };
  const attempt = 1;
  const browser = await connect();
  const { page, tabCount } = await ensureChatGptPage(browser.contexts());
  await establishFreshTemporaryUnpersonalized(page);
  ...
  if (result.recoveredExistingSubmission) throw new Error("SYNTHETIC_NORMAL_PATH_USED_RECOVERY");
  const acceptance = {
    schemaVersion: 1,
    studyId: STUDY_ID,
    status: "PASS",
    ...
    responseCapturedNormally: true,
    freshTemporaryConversationAfterward: nextChat.freshConversation,
    postCompletionSoakMilliseconds: 30_000,
    browserHealthyThroughSoak: true,
    responseSha256: result.responseSha256,
    acceptedAt: now(),
  };
  await writePrivateJson(acceptancePath, acceptance);
  process.stdout.write(`${JSON.stringify({ status: "SYNTHETIC_NORMAL_PATH_ACCEPTANCE_PASS", ...acceptance })}\n`);
  process.exit(0);
}
...
try {
  ...
  else if (command === "deterministic-supervision") await deterministicSupervisionAcceptance();
  else if (command === "synthetic-normal-path") await syntheticNormalPathAcceptance();
