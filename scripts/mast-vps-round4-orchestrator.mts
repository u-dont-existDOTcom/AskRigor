...
import { generationCaptureSchema } from "../evaluation/mast/src/fresh-validation-round-4.js";
import { assertVpsPacketTransferEligible, VPS_PRIVATE_ROOT } from "../evaluation/mast/src/vps-cdp-transport-round-4.js";
...
const REMOTE_SCRIPT = `${REMOTE_APP}/mast-vps-cdp-generation-round-4.mjs`;
...
async function preserveTerminalDisposition(artifactRoot: string, value: Record<string, unknown>) {
  const path = join(artifactRoot, "disposition/round-4-terminal-indeterminate.json");
  const ambiguityPath = join(artifactRoot, "disposition/round-4-post-send-ambiguity.json");
...
async function runtimeHashes(repositoryRoot: string) {
  const paths = {
    ...
    transportScriptSha256: "scripts/mast-vps-cdp-generation-round-4.mjs",
  } as const;
...
async function writeExpectedRuntimeHashes(repositoryRoot: string, artifactRoot: string) {
  ...
  const environment = await readJson(join(repositoryRoot, "evaluation/mast/fresh-validation-round-4-environment.json"));
  if (canonicalJson(observed) !== canonicalJson(environment.runtimeFileHashes)) {
    throw new Error("ROUND_4_RUNTIME_HASH_MANIFEST_DRIFT");
  }
...
async function installRuntime() {
  ...
  await run("scp", scpArgs(key, [
    ...
    join(repositoryRoot, "scripts/mast-vps-cdp-generation-round-4.mjs"),
  ...
  if (receipt.status !== "ROUND_4_RUNTIME_VERIFIED") throw new Error("ROUND_4_RUNTIME_VERIFICATION_FAILED");
  ...
  process.stdout.write(`${JSON.stringify({ status: "ROUND_4_RUNTIME_INSTALLED", browserService: BROWSER_SERVICE, cdpEndpoint: "http://127.0.0.1:9224", runtimeHashes: await runtimeHashes(repositoryRoot) })}\n`);
}
...
async function buildPacketManifest(artifactRoot: string) {
  ...
  if (packetMap?.studyId !== STUDY_ID || !Array.isArray(packetMap.records) || packetMap.records.length !== 120) {
    throw new Error("ROUND_4_PACKET_MAP_INVALID");
  }
  const records = [];
  for (const record of [...packetMap.records].sort((left, right) => left.sequence - right.sequence)) {
    ...
    if (sourceSha256 !== record.exactInputSha256 || bytes.byteLength !== record.inputUtf8Bytes) {
      throw new Error(`ROUND_4_PACKET_SOURCE_MISMATCH_${record.sequence}`);
    }
    records.push({
...
async function runDeterministicSupervision() {
  ...
  if (before.browserMainPid === after.browserMainPid) throw new Error("ROUND_4_PRE_SEND_SYSTEMD_RESTART_NOT_OBSERVED");
  ...
  if (checks.status !== "DETERMINISTIC_SUPERVISION_PASS" || !xrdpIndependent) {
    throw new Error("ROUND_4_DETERMINISTIC_SUPERVISION_INVALID");
  }
  ...
  process.stdout.write(`${JSON.stringify({ status: "ROUND_4_DETERMINISTIC_SUPERVISION_PASS", ...receipt })}\n`);
}
...
async function runSyntheticNormalPath() {
  ...
  if (acceptance.studyId !== STUDY_ID || acceptance.status !== "PASS"
    ...
    throw new Error("ROUND_4_SYNTHETIC_NORMAL_PATH_ACCEPTANCE_INVALID");
  }
  process.stdout.write(`${JSON.stringify({ status: "ROUND_4_SYNTHETIC_NORMAL_PATH_PASS", acceptanceSha256: sha256(await readFile(join(localAcceptanceDirectory, "acceptance.json"))), browserPid: acceptance.browserPidAfter })}\n`);
}
...
async function captureOne(input: {
  ...
  try {
    ...
    try {
      ...
      await writePrivate(join(artifactRoot, "disposition/round-4-post-send-ambiguity.json"),
  ...
  await run(process.execPath, [
    "--import", "tsx", join(repositoryRoot, "scripts/mast-fresh-validation-round-4.mts"), "capture-generation",
...
async function buildJudgeManifest(artifactRoot: string, judge: string) {
  ...
  if (schedule.studyId !== STUDY_ID || !Array.isArray(schedule.entries)
    ...
    throw new Error("ROUND_4_JUDGE_SCHEDULE_INVALID");
  }
  const records = [];
  for (const entry of schedule.entries) {
    ...
    if (sourceSha256 !== entry.exactPacketSha256 || !/^[0-9a-f]{64}$/u.test(entry.exactGenerationOutputSha256)) {
      throw new Error(`ROUND_4_JUDGE_PACKET_SOURCE_MISMATCH_${entry.opaqueResponseId}`);
    }
...
async function transferJudgingPackets() {
  ...
  if (receipt.studyId !== STUDY_ID || receipt.judge !== judge || receipt.records.length !== manifest.records.length
    ...
    throw new Error("ROUND_4_JUDGE_PACKET_TRANSFER_RECEIPT_INVALID");
  }
  process.stdout.write(`${JSON.stringify({ status: "ROUND_4_JUDGE_PACKET_TRANSFER_COMPLETE", judge, packetCount: manifest.records.length, receiptSha256: sha256(await readFile(localReceipt)) })}\n`);
}
...
async function captureJudgeOne(input: { repositoryRoot: string; mastGitDirectory: string;
  ...
  if (transport.state !== "RESPONSE_COMPLETE" || transport.sourceSha256 !== record.expectedSha256
    ...
    throw new Error("ROUND_4_JUDGMENT_TRANSPORT_RECEIPT_INVALID");
  }
  await run(process.execPath, ["--import", "tsx", join(repositoryRoot, "scripts/mast-fresh-validation-round-4.mts"),
