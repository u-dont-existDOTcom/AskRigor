...
import { generationTransportReceiptSchema } from "../evaluation/mast/src/chatgpt-browser-transport-round-5.js";
import {
  appendExposureEvent,
  benchmarkConflictReviewSchema,
  buildMachineSummary,
  createRound5Dispatch,
  detectPrimaryDisagreements,
  expectedBenchmarkConflictReviews,
  exposureLedgerSchema,
  generationCaptureSchema,
  generationProviderReceiptSchema,
  judgmentCaptureSchema,
  makeFailureReceipt,
  providerReceiptSchema,
  reconcileJudgments,
  ROUND_5_GENERATION_COUNT,
  ROUND_5_PRIMARY_JUDGMENT_COUNT,
  ROUND_5_STUDY_ID,
  scoreRound5,
  sha256,
} from "../evaluation/mast/src/fresh-validation-round-5.js";
...
function requiredArgument(name: string): string {
  ...
  if (!value) throw new Error(`ROUND_5_ARGUMENT_REQUIRED:${name}`);
  return value;
}
...
async function ensurePrivateRoot(path: string): Promise<string> {
  if (!isAbsolute(path)) throw new Error("ROUND_5_ARTIFACT_ROOT_NOT_ABSOLUTE");
  ...
  if (real === repository || real.startsWith(`${repository}/`)
    ...
    throw new Error("ROUND_5_ARTIFACT_ROOT_INVALID");
  }
  return real;
}
...
async function verifyFrozenSources(mastGitDirectory: string): Promise<JsonObject> {
  ...
  if (environment.studyId !== ROUND_5_STUDY_ID || preregistration.studyId !== ROUND_5_STUDY_ID
    || familyManifest.studyId !== ROUND_5_STUDY_ID) {
    throw new Error("ROUND_5_STUDY_ID_MISMATCH");
  }
  const branch = await gitText(root, "branch", "--show-current");
  if (activeTask.taskId !== ROUND_5_STUDY_ID || activeTask.exclusive !== true
    || activeTask.requiredBranch !== branch) {
    throw new Error("ROUND_5_ACTIVE_TASK_OR_BRANCH_MISMATCH");
  }
  ...
  if (identities.mastCommit !== environment.mast.commit || identities.mastTree !== environment.mast.tree) {
    throw new Error("ROUND_5_MAST_SOURCE_DRIFT");
  }
  ...
  if (sha256(await readFile(familyManifestPath)) !== preregistration.bindings.familyManifestSha256
    ...
    throw new Error("ROUND_5_PREREGISTRATION_BINDING_DRIFT");
  }
  ...
  if (JSON.stringify(ids) !== JSON.stringify(familyManifest.allFamilies)) {
    throw new Error("ROUND_5_IDENTIFIER_ONLY_SOURCE_MISMATCH");
  }
  return {
    identities,
...
async function sealFreeze(mastGitDirectory: string, artifactRoot: string, ciReceiptPath: string) {
  ...
  if (status !== "") throw new Error("ROUND_5_FREEZE_WORKTREE_NOT_CLEAN");
  ...
  if (ciReceipt.commit !== verification.identities.askRigorCommit) {
    throw new Error("ROUND_5_CI_COMMIT_MISMATCH");
  }
  ...
  if (requiredChecks.some((name) => !ciReceipt.checks.some((check) => check.name === name))) {
    throw new Error("ROUND_5_REQUIRED_CI_CHECK_MISSING");
  }
  ...
  if (seed.byteLength !== 32 || sha256(seed) !== preregistration.design.dispatchSeedSha256) {
    throw new Error("ROUND_5_DISPATCH_SEED_MISMATCH");
  }
  const familyManifest = await readJson(familyManifestPath);
  const schedule = createRound5Dispatch(seed, familyManifest.validationFamilies);
  if (schedule.length !== ROUND_5_GENERATION_COUNT) throw new Error("ROUND_5_DISPATCH_COUNT_INVALID");
  const privateMap = {
    schemaVersion: 1,
    studyId: ROUND_5_STUDY_ID,
    records: schedule,
  ...
  let ledger = exposureLedgerSchema.parse({ schemaVersion: 1, studyId: ROUND_5_STUDY_ID, events: [] });
  for (const familyId of familyManifest.allFamilies) {
    ...
    if (familyManifest.excludedDevelopmentFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        artifactSha256: null,
        note: "Excluded from Round 5 because the family was used in original development or calibration.",
      });
    }
    if (familyManifest.excludedRound1GeneratedFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        artifactSha256: null,
        note: "Excluded from Round 5 because Round 1 generated at least one response for this family.",
      });
    }
    if (familyManifest.excludedRound2ExposedFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        note: "Excluded from Round 5 because Round 2 exposed the family through generation.",
      });
    }
    if (familyManifest.excludedRound4ExposedFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        eventId: `round4-exposed:${familyId}`,
        familyId,
        kind: "GENERATED_RESPONSE_EXPOSURE",
        actor: "MODEL",
        stage: "HISTORICAL_ROUND_4_GENERATION",
        occurredAt: preregistration.historicalExposure.round4TerminalRecordedAt,
        artifactSha256: null,
        note: "Excluded from Round 5 because Round 4 completed a provider response for this family before the frozen capture-admission failure terminated the study.",
      });
    }
  }
  ...
  const receipt = {
    ...
    reserveFamilyCount: familyManifest.reserveFamilies.length,
    reserveStatus: familyManifest.reserveStatus.status,
...
function parseMastPrompts(items: string): Map<string, string> {
  ...
  for (const line of items.split("\n").filter(Boolean)) {
    ...
    if (typeof item.id !== "string" || typeof item.prompt !== "string" || prompts.has(item.id)) {
      throw new Error("ROUND_5_MAST_ITEMS_SCHEMA_INVALID");
    }
...
async function prepareGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (freeze.response1Generated !== false || freeze.payloadOpened !== false) {
    throw new Error("ROUND_5_FREEZE_RECEIPT_INVALID");
  }
  const environment = await readJson(environmentPath);
  const familyManifest = await readJson(familyManifestPath);
  ...
  if ([...selected].some((familyId) => !prompts.has(familyId))) {
    throw new Error("ROUND_5_SELECTED_PROMPT_MISSING");
  }
  const records = [];
  ...
  const packetMap = { schemaVersion: 1, studyId: ROUND_5_STUDY_ID, records };
...
async function status(mastGitDirectory: string, artifactRoot: string) {
  ...
  return {
    studyId: ROUND_5_STUDY_ID,
    generation: { completed: generation.length, planned: ROUND_5_GENERATION_COUNT },
    J1: { completed: j1.length, planned: ROUND_5_GENERATION_COUNT },
    J2: { completed: j2.length, planned: ROUND_5_GENERATION_COUNT },
...
export function admitGenerationCapture(input: {
  ...
  if (input.output.byteLength === 0) throw new Error("ROUND_5_GENERATION_OUTPUT_EMPTY");
  const provider = generationProviderReceiptSchema.parse(input.providerValue);
  const transportText = Buffer.from(input.transportBytes).toString("utf8");
  const transport = generationTransportReceiptSchema.parse(JSON.parse(transportText));
  if (transport.opaqueInputId !== input.opaqueInputId
    || transport.sourceSha256 !== input.exactInputSha256
    || transport.state !== "RESPONSE_COMPLETE") {
    throw new Error("ROUND_5_GENERATION_TRANSPORT_RECEIPT_INVALID");
  }
  return generationCaptureSchema.parse({
    schemaVersion: 1,
    studyId: ROUND_5_STUDY_ID,
    opaqueInputId: input.opaqueInputId,
    exactInputSha256: input.exactInputSha256,
    exactOutputSha256: sha256(input.output),
    outputUtf8Bytes: input.output.byteLength,
    transportReceiptSha256: sha256(input.transportBytes),
    provider,
  });
}
...
async function captureGeneration(mastGitDirectory: string, artifactRoot: string, opaqueInputId: string,
  ...
  if (!record) throw new Error("ROUND_5_GENERATION_SLOT_UNKNOWN");
  ...
  const capture = admitGenerationCapture({
    opaqueInputId, exactInputSha256: record.exactInputSha256, output, providerValue, transportBytes,
  });
...
async function sealGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (captures.length !== ROUND_5_GENERATION_COUNT) throw new Error("ROUND_5_GENERATION_COVERAGE_INCOMPLETE");
  const ids = captures.map(({ opaqueInputId }) => opaqueInputId);
  if (new Set(ids).size !== ROUND_5_GENERATION_COUNT) throw new Error("ROUND_5_GENERATION_ID_DUPLICATE");
  const receipt = {
    schemaVersion: 1,
    studyId: ROUND_5_STUDY_ID,
...
async function buildBlindPackets(mastGitDirectory: string, artifactRoot: string) {
  ...
  const mapping = { schemaVersion: 1, studyId: ROUND_5_STUDY_ID, records: mappings };
...
async function captureJudgment(mastGitDirectory: string, artifactRoot: string, judge: "J1" | "J2" | "J3",
  ...
  if (!slot) throw new Error("ROUND_5_JUDGMENT_SLOT_UNKNOWN");
  ...
  if (attempt > 3) throw new Error("ROUND_5_JUDGMENT_ATTEMPT_CEILING_EXHAUSTED");
  try {
...
async function buildAdjudication(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (j1Paths.length !== ROUND_5_GENERATION_COUNT || j2Paths.length !== ROUND_5_GENERATION_COUNT) {
    throw new Error("ROUND_5_PRIMARY_JUDGMENTS_INCOMPLETE");
  }
  ...
  const schedule = { schemaVersion: 1, studyId: ROUND_5_STUDY_ID, entries };
...
function markdownReport(summary: any, familyManifest: any): string {
  ...
  const countSummary = (field: "omissions" | "commissions") => ["A", "B", "C", "D"]
    ...
    .join(", ");
  return `# MAST Fresh Validation Round 5 final report\n\n`
    ...
    + `(identifier-only in Round 5; historically mechanically read in Round 1, never generated or judge-exposed; status ${familyManifest.reserveStatus.status}).\n\n`
...
async function finalize(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (generationPaths.length !== ROUND_5_GENERATION_COUNT || j1Paths.length !== ROUND_5_GENERATION_COUNT
    || j2Paths.length !== ROUND_5_GENERATION_COUNT) throw new Error("ROUND_5_FINAL_COVERAGE_INCOMPLETE");
  ...
  if (JSON.stringify(observedReviews) !== JSON.stringify(expectedReviews)) {
    throw new Error("ROUND_5_BENCHMARK_CONFLICT_REVIEW_COVERAGE_INVALID");
  }
  const result = scoreRound5({
    generation: generations.map(({ opaqueInputId, exactOutputSha256 }) => ({ opaqueInputId, exactOutputSha256 })),
    mapping: mapping.records,
  ...
  const summary = buildMachineSummary({
    result,
    plannedResponses: ROUND_5_GENERATION_COUNT,
...
async function main(): Promise<void> {
  ...
  if (command === "create-seed") result = await createSeed(requiredArgument("--artifact-root"));
    ...
    requiredArgument("--artifact-root"));
  else throw new Error("ROUND_5_COMMAND_INVALID");
    ...
    if (artifactRoot && mastGitDirectory) {
      try {
        ...
        const receipt = makeFailureReceipt({
          ...
          failureCode: "ROUND_5_STAGE_FAILED",
          validationErrors: [error instanceof Error ? error.message : "unknown error"],
          timestamp: new Date().toISOString(),
          source: identities,
          stoppedBeforeDownstreamArtifact: true,
