...
import {
  ...
  ROUND_2_PRIMARY_JUDGMENT_COUNT,
  ROUND_2_STUDY_ID,
  scoreRound2,
  sha256,
...
const preregistrationPath = resolve(root, "evaluation/mast/fresh-validation-round-2-preregistration.json");
const environmentPath = resolve(root, "evaluation/mast/fresh-validation-round-2-environment.json");
const familyManifestPath = resolve(root, "evaluation/mast/fresh-validation-round-2-family-manifest.json");
const primaryJudgePromptPath = resolve(root, "evaluation/mast/prompts/fresh-round-2-primary-judge.md");
const adjudicatorPromptPath = resolve(root, "evaluation/mast/prompts/fresh-round-2-adjudicator.md");
...
function requiredArgument(name: string): string {
  ...
  if (!value) throw new Error(`ROUND_2_ARGUMENT_REQUIRED:${name}`);
  return value;
}
...
async function ensurePrivateRoot(path: string): Promise<string> {
  if (!isAbsolute(path)) throw new Error("ROUND_2_ARTIFACT_ROOT_NOT_ABSOLUTE");
  ...
  if (real === repository || real.startsWith(`${repository}/`)
    ...
    throw new Error("ROUND_2_ARTIFACT_ROOT_INVALID");
  }
  return real;
}
...
async function verifyFrozenSources(mastGitDirectory: string): Promise<JsonObject> {
  ...
  if (environment.studyId !== ROUND_2_STUDY_ID || preregistration.studyId !== ROUND_2_STUDY_ID
    || familyManifest.studyId !== ROUND_2_STUDY_ID) {
    throw new Error("ROUND_2_STUDY_ID_MISMATCH");
  }
  const branch = await gitText(root, "branch", "--show-current");
  if (activeTask.taskId !== ROUND_2_STUDY_ID || activeTask.exclusive !== true
    || activeTask.requiredBranch !== branch) {
    throw new Error("ROUND_2_ACTIVE_TASK_OR_BRANCH_MISMATCH");
  }
  ...
  if (identities.mastCommit !== environment.mast.commit || identities.mastTree !== environment.mast.tree) {
    throw new Error("ROUND_2_MAST_SOURCE_DRIFT");
  }
  ...
  if (sha256(universalBytes) !== environment.askRigor.universal.sha256
    ...
    throw new Error("ROUND_2_PROTOCOL_SOURCE_DRIFT");
  }
  for (const [relativePath, expected] of Object.entries(environment.executableHashManifest) as Array<[string, string]>) {
    if (sha256(await readFile(resolve(root, relativePath))) !== expected) {
      throw new Error(`ROUND_2_EXECUTABLE_HASH_DRIFT:${relativePath}`);
    }
  }
  if (sha256(await readFile(familyManifestPath)) !== preregistration.bindings.familyManifestSha256
    ...
    throw new Error("ROUND_2_PREREGISTRATION_BINDING_DRIFT");
  }
  ...
  if (JSON.stringify(ids) !== JSON.stringify(familyManifest.allFamilies)) {
    throw new Error("ROUND_2_IDENTIFIER_ONLY_SOURCE_MISMATCH");
  }
  return {
    identities,
...
async function verifyRuntimeBinding(mastGitDirectory: string, artifactRoot: string): Promise<JsonObject> {
  ...
  if (freeze.studyId !== ROUND_2_STUDY_ID
    ...
    throw new Error("ROUND_2_RUNTIME_SOURCE_BINDING_MISMATCH");
  }
  return verification;
}
...
async function sealFreeze(mastGitDirectory: string, artifactRoot: string, ciReceiptPath: string) {
  ...
  if (status !== "") throw new Error("ROUND_2_FREEZE_WORKTREE_NOT_CLEAN");
  ...
  if (ciReceipt.commit !== verification.identities.askRigorCommit) {
    throw new Error("ROUND_2_CI_COMMIT_MISMATCH");
  }
  ...
  if (requiredChecks.some((name) => !ciReceipt.checks.some((check) => check.name === name))) {
    throw new Error("ROUND_2_REQUIRED_CI_CHECK_MISSING");
  }
  ...
  if (seed.byteLength !== 32 || sha256(seed) !== preregistration.design.dispatchSeedSha256) {
    throw new Error("ROUND_2_DISPATCH_SEED_MISMATCH");
  }
  const familyManifest = await readJson(familyManifestPath);
  const schedule = createRound2Dispatch(seed, familyManifest.validationFamilies);
  if (schedule.length !== ROUND_2_GENERATION_COUNT) throw new Error("ROUND_2_DISPATCH_COUNT_INVALID");
  const privateMap = {
    schemaVersion: 1,
    studyId: ROUND_2_STUDY_ID,
    records: schedule,
  ...
  let ledger = exposureLedgerSchema.parse({ schemaVersion: 1, studyId: ROUND_2_STUDY_ID, events: [] });
  ...
  const receipt = {
    ...
    confirmationFamilyCount: familyManifest.confirmationFamilies.length,
...
function parseMastPrompts(items: string): Map<string, string> {
  ...
  for (const line of items.split("\n").filter(Boolean)) {
    ...
    if (typeof item.id !== "string" || typeof item.prompt !== "string" || prompts.has(item.id)) {
      throw new Error("ROUND_2_MAST_ITEMS_SCHEMA_INVALID");
    }
...
async function prepareGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (freeze.response1Generated !== false || freeze.payloadOpened !== false) {
    throw new Error("ROUND_2_FREEZE_RECEIPT_INVALID");
  }
  const environment = await readJson(environmentPath);
  const familyManifest = await readJson(familyManifestPath);
  ...
  const selected = new Set(familyManifest.validationFamilies);
  if ([...selected].some((familyId) => !prompts.has(familyId))) {
    throw new Error("ROUND_2_SELECTED_PROMPT_MISSING");
  }
  const records = [];
  ...
  const packetMap = { schemaVersion: 1, studyId: ROUND_2_STUDY_ID, records };
  ...
  const receipt = {
    ...
    confirmationPayloadSemanticallyExposed: false,
...
async function status(mastGitDirectory: string, artifactRoot: string) {
  ...
  return {
    studyId: ROUND_2_STUDY_ID,
    generation: { completed: generation.length, planned: ROUND_2_GENERATION_COUNT },
    J1: { completed: j1.length, planned: ROUND_2_GENERATION_COUNT },
    J2: { completed: j2.length, planned: ROUND_2_GENERATION_COUNT },
...
async function sealGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (captures.length !== ROUND_2_GENERATION_COUNT) throw new Error("ROUND_2_GENERATION_COVERAGE_INCOMPLETE");
  const ids = captures.map(({ opaqueInputId }) => opaqueInputId);
  if (new Set(ids).size !== ROUND_2_GENERATION_COUNT) throw new Error("ROUND_2_GENERATION_ID_DUPLICATE");
  const receipt = {
    schemaVersion: 1,
    studyId: ROUND_2_STUDY_ID,
...
async function buildBlindPackets(mastGitDirectory: string, artifactRoot: string) {
  ...
  const mapping = { schemaVersion: 1, studyId: ROUND_2_STUDY_ID, records: mappings };
  const mappingSha256 = await writePrivate(resolve(privateRoot, "evaluation/sealed-condition-map.json"), json(mapping));
  const schedule = {
    schemaVersion: 1,
    studyId: ROUND_2_STUDY_ID,
    entries: mappings.map(({ opaqueResponseId, exactPacketSha256, packetFile }) =>
      ({ opaqueResponseId, exactPacketSha256, packetFile })),
  };
...
async function nextJudgment(mastGitDirectory: string, artifactRoot: string, judge: "J1" | "J2" | "J3") {
  const privateRoot = await ensurePrivateRoot(artifactRoot);
  await verifyRuntimeBinding(mastGitDirectory, privateRoot);
  if (!["J1", "J2", "J3"].includes(judge)) throw new Error("ROUND_2_JUDGE_INVALID");
...
async function captureJudgment(mastGitDirectory: string, artifactRoot: string, judge: "J1" | "J2" | "J3",
  ...
  if (!slot) throw new Error("ROUND_2_JUDGMENT_SLOT_UNKNOWN");
  ...
  if (attempt > 3) throw new Error("ROUND_2_JUDGMENT_ATTEMPT_CEILING_EXHAUSTED");
  try {
...
async function buildAdjudication(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (j1Paths.length !== ROUND_2_GENERATION_COUNT || j2Paths.length !== ROUND_2_GENERATION_COUNT) {
    throw new Error("ROUND_2_PRIMARY_JUDGMENTS_INCOMPLETE");
  }
  ...
  const schedule = { schemaVersion: 1, studyId: ROUND_2_STUDY_ID, entries };
...
function markdownReport(summary: any, familyManifest: any): string {
  ...
  return `# MAST Fresh Validation Round 2 final report\n\n`
    ...
    + `Confirmation reserve: ${familyManifest.confirmationFamilies.join(", ")} `
    + `(identifier-only in Round 2; historically mechanically read in Round 1, never generated or judge-exposed).\n`;
}
...
async function finalize(mastGitDirectory: string, artifactRoot: string) {
      ...
      const walk = async (path: string): Promise<string[]> => {
        try {
          ...
          return (await Promise.all(entries.map((entry) => entry.isDirectory()
            ...
            : Promise.resolve(entry.name.endsWith("-receipt.json") ? [resolve(path, entry.name)] : [])))).flat();
        } catch (error: any) { if (error?.code === "ENOENT") return []; throw error; }
      };
      return walk(resolve(privateRoot, "evaluation"));
    })(),
    listPrivateJson(resolve(privateRoot, "clinical/conflict-reviews")),
  ]);
  if (generationPaths.length !== ROUND_2_GENERATION_COUNT || j1Paths.length !== ROUND_2_GENERATION_COUNT
    || j2Paths.length !== ROUND_2_GENERATION_COUNT) throw new Error("ROUND_2_FINAL_COVERAGE_INCOMPLETE");
  ...
  const flagged = new Set(reconciliation.final.filter(({ benchmarkTargetConflict }) => benchmarkTargetConflict)
    .map(({ opaqueResponseId }) => opaqueResponseId));
  if (flagged.size > 0 && reviews.length === 0) throw new Error("ROUND_2_BENCHMARK_CONFLICT_REVIEW_INCOMPLETE");
  ...
  const summary = buildMachineSummary({
    result,
    plannedResponses: ROUND_2_GENERATION_COUNT,
...
async function main(): Promise<void> {
  ...
  if (command === "create-seed") result = await createSeed(requiredArgument("--artifact-root"));
    ...
    requiredArgument("--artifact-root"));
  else throw new Error("ROUND_2_COMMAND_INVALID");
    ...
    if (artifactRoot && mastGitDirectory) {
      try {
        ...
        const receipt = makeFailureReceipt({
          ...
          failureCode: "ROUND_2_STAGE_FAILED",
          validationErrors: [error instanceof Error ? error.message : "unknown error"],
          timestamp: new Date().toISOString(),
          source: identities,
          stoppedBeforeDownstreamArtifact: true,
