...
const MCP_SPARK_PARAGRAPH = "Broad treatment/avoid-surgery, substantial YouTube corpus: require supplied `gemini_youtube_candidate_handoff`; validate identities. Absence blocks completion; skill/validator existence or native results cannot substitute. Reject only literal not-found/not-visible or verified mismatch; keep others unresolved. Screen all leads. Summaries are unverified cues, not evidence." as const;
const CUSTOM_GPT_SPARK_PARAGRAPH = "For broad treatment/avoid-surgery with substantial YouTube results, require supplied `gemini_youtube_candidate_handoff`; call `validate_gemini_youtube_candidate_handoff`. If it, `get_youtube_transcript`, or `assess_treatment_landscape_coverage` is absent, say Action setup is out of date and stop; no substitute/final comparison. Screen all leads. Without captions say the summary was not checked against a transcript; never use it as evidence." as const;
