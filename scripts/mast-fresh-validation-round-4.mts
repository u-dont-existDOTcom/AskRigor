...
import { generationTransportReceiptSchema } from "../evaluation/mast/src/chatgpt-browser-transport-round-4.js";
import {
  appendExposureEvent,
  ...
  ROUND_4_PRIMARY_JUDGMENT_COUNT,
  ROUND_4_STUDY_ID,
  scoreRound4,
  sha256,
...
const preregistrationPath = resolve(root, "evaluation/mast/fresh-validation-round-4-preregistration.json");
const environmentPath = resolve(root, "evaluation/mast/fresh-validation-round-4-environment.json");
const familyManifestPath = resolve(root, "evaluation/mast/fresh-validation-round-4-family-manifest.json");
const primaryJudgePromptPath = resolve(root, "evaluation/mast/prompts/fresh-round-4-primary-judge.md");
const adjudicatorPromptPath = resolve(root, "evaluation/mast/prompts/fresh-round-4-adjudicator.md");
...
function requiredArgument(name: string): string {
  ...
  if (!value) throw new Error(`ROUND_4_ARGUMENT_REQUIRED:${name}`);
  return value;
}
...
async function ensurePrivateRoot(path: string): Promise<string> {
  if (!isAbsolute(path)) throw new Error("ROUND_4_ARTIFACT_ROOT_NOT_ABSOLUTE");
  ...
  if (real === repository || real.startsWith(`${repository}/`)
    ...
    throw new Error("ROUND_4_ARTIFACT_ROOT_INVALID");
  }
  return real;
}
...
async function verifyFrozenSources(mastGitDirectory: string): Promise<JsonObject> {
  ...
  if (environment.studyId !== ROUND_4_STUDY_ID || preregistration.studyId !== ROUND_4_STUDY_ID
    || familyManifest.studyId !== ROUND_4_STUDY_ID) {
    throw new Error("ROUND_4_STUDY_ID_MISMATCH");
  }
  const branch = await gitText(root, "branch", "--show-current");
  if (activeTask.taskId !== ROUND_4_STUDY_ID || activeTask.exclusive !== true
    || activeTask.requiredBranch !== branch) {
    throw new Error("ROUND_4_ACTIVE_TASK_OR_BRANCH_MISMATCH");
  }
  ...
  if (identities.mastCommit !== environment.mast.commit || identities.mastTree !== environment.mast.tree) {
    throw new Error("ROUND_4_MAST_SOURCE_DRIFT");
  }
  ...
  if (sha256(universalBytes) !== environment.askRigor.universal.sha256
    ...
    throw new Error("ROUND_4_PROTOCOL_SOURCE_DRIFT");
  }
  for (const [relativePath, expected] of Object.entries(environment.executableHashManifest) as Array<[string, string]>) {
    if (sha256(await readFile(resolve(root, relativePath))) !== expected) {
      throw new Error(`ROUND_4_EXECUTABLE_HASH_DRIFT:${relativePath}`);
    }
  }
  if (sha256(await readFile(familyManifestPath)) !== preregistration.bindings.familyManifestSha256
    ...
    throw new Error("ROUND_4_PREREGISTRATION_BINDING_DRIFT");
  }
  ...
  if (JSON.stringify(ids) !== JSON.stringify(familyManifest.allFamilies)) {
    throw new Error("ROUND_4_IDENTIFIER_ONLY_SOURCE_MISMATCH");
  }
  return {
    identities,
...
async function verifyRuntimeBinding(mastGitDirectory: string, artifactRoot: string): Promise<JsonObject> {
  ...
  if (freeze.studyId !== ROUND_4_STUDY_ID
    ...
    throw new Error("ROUND_4_RUNTIME_SOURCE_BINDING_MISMATCH");
  }
  return verification;
}
...
async function sealFreeze(mastGitDirectory: string, artifactRoot: string, ciReceiptPath: string) {
  ...
  if (status !== "") throw new Error("ROUND_4_FREEZE_WORKTREE_NOT_CLEAN");
  ...
  if (ciReceipt.commit !== verification.identities.askRigorCommit) {
    throw new Error("ROUND_4_CI_COMMIT_MISMATCH");
  }
  ...
  if (requiredChecks.some((name) => !ciReceipt.checks.some((check) => check.name === name))) {
    throw new Error("ROUND_4_REQUIRED_CI_CHECK_MISSING");
  }
  ...
  if (seed.byteLength !== 32 || sha256(seed) !== preregistration.design.dispatchSeedSha256) {
    throw new Error("ROUND_4_DISPATCH_SEED_MISMATCH");
  }
  ...
  if (schedule.length !== ROUND_4_GENERATION_COUNT) throw new Error("ROUND_4_DISPATCH_COUNT_INVALID");
  const privateMap = {
    schemaVersion: 1,
    studyId: ROUND_4_STUDY_ID,
    records: schedule,
  ...
  let ledger = exposureLedgerSchema.parse({ schemaVersion: 1, studyId: ROUND_4_STUDY_ID, events: [] });
  for (const familyId of familyManifest.allFamilies) {
    ...
    if (familyManifest.excludedDevelopmentFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        artifactSha256: null,
        note: "Excluded from Round 4 because the family was used in original development or calibration.",
      });
    }
    if (familyManifest.excludedRound1GeneratedFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        artifactSha256: null,
        note: "Excluded from Round 4 because Round 1 generated at least one response for this family.",
      });
    }
    if (familyManifest.excludedRound2ExposedFamilies.includes(familyId)) {
      ledger = appendExposureEvent(ledger, {
        ...
        note: "Excluded from Round 4 because Round 2 exposed the family through generation.",
      });
    }
  }
...
function parseMastPrompts(items: string): Map<string, string> {
  ...
  for (const line of items.split("\n").filter(Boolean)) {
    ...
    if (typeof item.id !== "string" || typeof item.prompt !== "string" || prompts.has(item.id)) {
      throw new Error("ROUND_4_MAST_ITEMS_SCHEMA_INVALID");
    }
...
async function prepareGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (freeze.response1Generated !== false || freeze.payloadOpened !== false) {
    throw new Error("ROUND_4_FREEZE_RECEIPT_INVALID");
  }
  const environment = await readJson(environmentPath);
  const familyManifest = await readJson(familyManifestPath);
  ...
  if ([...selected].some((familyId) => !prompts.has(familyId))) {
    throw new Error("ROUND_4_SELECTED_PROMPT_MISSING");
  }
  const records = [];
  ...
  const packetMap = { schemaVersion: 1, studyId: ROUND_4_STUDY_ID, records };
...
async function status(mastGitDirectory: string, artifactRoot: string) {
  ...
  return {
    studyId: ROUND_4_STUDY_ID,
    generation: { completed: generation.length, planned: ROUND_4_GENERATION_COUNT },
    J1: { completed: j1.length, planned: ROUND_4_GENERATION_COUNT },
    J2: { completed: j2.length, planned: ROUND_4_GENERATION_COUNT },
...
async function captureGeneration(mastGitDirectory: string, artifactRoot: string, opaqueInputId: string,
  ...
  if (!record) throw new Error("ROUND_4_GENERATION_SLOT_UNKNOWN");
  ...
  if (output.byteLength === 0) throw new Error("ROUND_4_GENERATION_OUTPUT_EMPTY");
  ...
  if (transport.opaqueInputId !== opaqueInputId || transport.sourceSha256 !== record.exactInputSha256
    ...
    throw new Error("ROUND_4_GENERATION_TRANSPORT_RECEIPT_INVALID");
  }
...
async function sealGeneration(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (captures.length !== ROUND_4_GENERATION_COUNT) throw new Error("ROUND_4_GENERATION_COVERAGE_INCOMPLETE");
  const ids = captures.map(({ opaqueInputId }) => opaqueInputId);
  if (new Set(ids).size !== ROUND_4_GENERATION_COUNT) throw new Error("ROUND_4_GENERATION_ID_DUPLICATE");
  const receipt = {
    schemaVersion: 1,
    studyId: ROUND_4_STUDY_ID,
...
async function buildBlindPackets(mastGitDirectory: string, artifactRoot: string) {
  ...
  const mapping = { schemaVersion: 1, studyId: ROUND_4_STUDY_ID, records: mappings };
...
async function nextJudgment(mastGitDirectory: string, artifactRoot: string, judge: "J1" | "J2" | "J3") {
  const privateRoot = await ensurePrivateRoot(artifactRoot);
  await verifyRuntimeBinding(mastGitDirectory, privateRoot);
  if (!["J1", "J2", "J3"].includes(judge)) throw new Error("ROUND_4_JUDGE_INVALID");
...
async function captureJudgment(mastGitDirectory: string, artifactRoot: string, judge: "J1" | "J2" | "J3",
  ...
  if (!slot) throw new Error("ROUND_4_JUDGMENT_SLOT_UNKNOWN");
  ...
  if (attempt > 3) throw new Error("ROUND_4_JUDGMENT_ATTEMPT_CEILING_EXHAUSTED");
  try {
...
async function buildAdjudication(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (j1Paths.length !== ROUND_4_GENERATION_COUNT || j2Paths.length !== ROUND_4_GENERATION_COUNT) {
    throw new Error("ROUND_4_PRIMARY_JUDGMENTS_INCOMPLETE");
  }
  ...
  const schedule = { schemaVersion: 1, studyId: ROUND_4_STUDY_ID, entries };
...
function markdownReport(summary: any, familyManifest: any): string {
  ...
  const countSummary = (field: "omissions" | "commissions") => ["A", "B", "C", "D"]
    ...
    .join(", ");
  return `# MAST Fresh Validation Round 4 final report\n\n`
    ...
    + `(identifier-only in Round 4; historically mechanically read in Round 1, never generated or judge-exposed; status ${familyManifest.reserveStatus.status}).\n\n`
...
async function finalize(mastGitDirectory: string, artifactRoot: string) {
  ...
  if (generationPaths.length !== ROUND_4_GENERATION_COUNT || j1Paths.length !== ROUND_4_GENERATION_COUNT
    || j2Paths.length !== ROUND_4_GENERATION_COUNT) throw new Error("ROUND_4_FINAL_COVERAGE_INCOMPLETE");
  ...
  if (JSON.stringify(observedReviews) !== JSON.stringify(expectedReviews)) {
    throw new Error("ROUND_4_BENCHMARK_CONFLICT_REVIEW_COVERAGE_INVALID");
  }
  ...
  const summary = buildMachineSummary({
    result,
    plannedResponses: ROUND_4_GENERATION_COUNT,
...
async function main(): Promise<void> {
  ...
  if (command === "create-seed") result = await createSeed(requiredArgument("--artifact-root"));
    ...
    requiredArgument("--artifact-root"));
  else throw new Error("ROUND_4_COMMAND_INVALID");
    ...
    if (artifactRoot && mastGitDirectory) {
      try {
        ...
        const receipt = makeFailureReceipt({
          ...
          failureCode: "ROUND_4_STAGE_FAILED",
          validationErrors: [error instanceof Error ? error.message : "unknown error"],
          timestamp: new Date().toISOString(),
          source: identities,
          stoppedBeforeDownstreamArtifact: true,
