...
import { generationCaptureSchema } from "../evaluation/mast/src/fresh-validation-round-3.js";
import { assertVpsPacketTransferEligible, VPS_PRIVATE_ROOT } from "../evaluation/mast/src/vps-cdp-transport-round-3.js";
...
const REMOTE_SCRIPT = `${REMOTE_APP}/mast-vps-cdp-generation-round-3.mjs`;
...
async function preserveTerminalDisposition(artifactRoot: string, value: Record<string, unknown>) {
  const path = join(artifactRoot, "disposition/round-3-terminal-indeterminate.json");
  const ambiguityPath = join(artifactRoot, "disposition/round-3-post-send-ambiguity.json");
...
async function runtimeHashes(repositoryRoot: string) {
  const paths = {
    ...
    transportScriptSha256: "scripts/mast-vps-cdp-generation-round-3.mjs",
  } as const;
...
async function writeExpectedRuntimeHashes(repositoryRoot: string, artifactRoot: string) {
  ...
  const environment = await readJson(join(repositoryRoot, "evaluation/mast/fresh-validation-round-3-environment.json"));
  if (canonicalJson(observed) !== canonicalJson(environment.runtimeFileHashes)) {
    throw new Error("ROUND_3_RUNTIME_HASH_MANIFEST_DRIFT");
  }
...
async function installRuntime() {
  ...
  await run("scp", scpArgs(key, [
    join(repositoryRoot, "deploy/vps/clone-round3-profile.sh"),
    join(repositoryRoot, "deploy/vps/brave-mast-round3.sh"),
    join(repositoryRoot, "deploy/vps/cdp-watchdog.sh"),
    join(repositoryRoot, "scripts/mast-vps-cdp-generation-round-3.mjs"),
  ...
  await run("ssh", sshArgs(key, host,
    `set -eu; install -m 0755 ${REMOTE_APP}/brave-mast-round3.sh /usr/local/lib/askrigor-mast-round3/brave-mast-round3.sh; install -m 0755 ${REMOTE_APP}/cdp-watchdog.sh /usr/local/lib/askrigor-mast-round3/cdp-watchdog.sh; if [ ! -d /home/cloudbrowser/.config/brave-mast-round3 ]; then chmod 0700 ${REMOTE_APP}/clone-round3-profile.sh; ${REMOTE_APP}/clone-round3-profile.sh; fi; test -f ${VPS_PRIVATE_ROOT}/environment/profile-genesis.json; chown cloudbrowser:cloudbrowser ${REMOTE_APP}/mast-vps-cdp-generation-round-3.mjs ${REMOTE_APP}/package.json ${REMOTE_APP}/package-lock.json ${REMOTE_RUNTIME_HASHES}; chmod 0700 ${REMOTE_APP}/mast-vps-cdp-generation-round-3.mjs; chmod 0600 ${REMOTE_APP}/package.json ${REMOTE_APP}/package-lock.json ${REMOTE_RUNTIME_HASHES}; runuser -u cloudbrowser -- env PATH=/home/cloudbrowser/.local/bin:/usr/bin:/bin /home/cloudbrowser/.local/bin/npm --prefix ${REMOTE_APP} ci --ignore-scripts; systemctl daemon-reload; systemctl enable --now ${DISPLAY_SERVICE} ${BROWSER_SERVICE} ${WATCHDOG_SERVICE}`), 10 * 60 * 1_000);
  ...
  const receipt = JSON.parse(verify.stdout.trim().split("\n").at(-1)!);
  if (receipt.status !== "ROUND_3_RUNTIME_VERIFIED") throw new Error("ROUND_3_RUNTIME_VERIFICATION_FAILED");
  await writePrivate(join(artifactRoot, "environment/vps-runtime-attestation.json"), Buffer.from(canonicalJson(receipt), "utf8"));
  const profileGenesisPath = join(artifactRoot, "environment/vps-profile-genesis.json");
  await run("scp", scpArgs(key, [`root@${host}:${VPS_PRIVATE_ROOT}/environment/profile-genesis.json`], profileGenesisPath));
  const profileGenesis = await readJson(profileGenesisPath);
  if (profileGenesis.studyId !== STUDY_ID || profileGenesis.sourceDestinationManifestEqual !== true
    || profileGenesis.owner !== USER || profileGenesis.destinationProfile !== "/home/cloudbrowser/.config/brave-mast-round3") {
    throw new Error("ROUND_3_PROFILE_GENESIS_INVALID");
  }
  process.stdout.write(`${JSON.stringify({ status: "ROUND_3_RUNTIME_INSTALLED", browserService: BROWSER_SERVICE, cdpEndpoint: "http://127.0.0.1:9224", runtimeHashes: await runtimeHashes(repositoryRoot) })}\n`);
}
...
async function buildPacketManifest(artifactRoot: string) {
  ...
  if (packetMap?.studyId !== STUDY_ID || !Array.isArray(packetMap.records) || packetMap.records.length !== 120) {
    throw new Error("ROUND_3_PACKET_MAP_INVALID");
  }
  const records = [];
  for (const record of [...packetMap.records].sort((left, right) => left.sequence - right.sequence)) {
    ...
    if (sourceSha256 !== record.exactInputSha256 || bytes.byteLength !== record.inputUtf8Bytes) {
      throw new Error(`ROUND_3_PACKET_SOURCE_MISMATCH_${record.sequence}`);
    }
    records.push({
...
async function runSyntheticCrashRecovery() {
  const repositoryRoot = resolve(requiredArgument("--repository-root"));
  const artifactRoot = resolve(requiredArgument("--artifact-root"));
  const key = resolve(requiredArgument("--ssh-key"));
  const host = requiredArgument("--host");
  await writeExpectedRuntimeHashes(repositoryRoot, artifactRoot);
  const remoteAcceptance = `${VPS_PRIVATE_ROOT}/acceptance/synthetic-durability`;
  await run("ssh", sshArgs(key, host,
    `set -eu; install -d -m 0700 -o cloudbrowser -g cloudbrowser ${remoteAcceptance}; if [ ! -f ${remoteAcceptance}/acceptance.json ]; then runuser -u cloudbrowser -- /home/cloudbrowser/.local/bin/node ${REMOTE_SCRIPT} synthetic-crash-recovery --workspace ${VPS_PRIVATE_ROOT} --expected-runtime-hashes ${REMOTE_RUNTIME_HASHES} >${remoteAcceptance}/worker.stdout 2>${remoteAcceptance}/worker.stderr & worker=$!; deadline=$((SECONDS + 90)); while [ ! -f ${remoteAcceptance}/sent.json ]; do kill -0 $worker; if [ $SECONDS -ge $deadline ]; then kill $worker || true; wait $worker || true; echo SYNTHETIC_SENT_RECEIPT_TIMEOUT >&2; exit 1; fi; sleep 0.1; done; systemctl kill --signal=KILL --kill-whom=main ${BROWSER_SERVICE}; wait $worker; fi`), RESPONSE_TIMEOUT_FOR_ORCHESTRATOR);
  const localAcceptanceDirectory = join(artifactRoot, "environment/vps-synthetic-crash-recovery");
  ...
  await run("scp", scpArgs(key, [
    ...
    `root@${host}:${remoteAcceptance}/recovery.json`,
  ...
  if (acceptance.studyId !== STUDY_ID || acceptance.status !== "PASS"
    || acceptance.exactEquality !== true || acceptance.systemdRestartObserved !== true
    || acceptance.exactUserMessageRecovered !== true || acceptance.responseRecoveredWithoutResend !== true
    || acceptance.modelVisibleLabel !== "GPT-5.6 Sol" || acceptance.reasoningVisibleLabel !== "Extra High"
    || acceptance.reasoningOrdinal !== "4 of 5" || acceptance.chatMode !== "TEMPORARY"
    || acceptance.personalization !== "UNPERSONALIZED") {
    throw new Error("ROUND_3_SYNTHETIC_CRASH_RECOVERY_ACCEPTANCE_INVALID");
  }
  process.stdout.write(`${JSON.stringify({ status: "ROUND_3_SYNTHETIC_CRASH_RECOVERY_PASS", acceptanceSha256: sha256(await readFile(join(localAcceptanceDirectory, "acceptance.json"))), browserPidBefore: acceptance.browserPidBefore, browserPidAfter: acceptance.browserPidAfter })}\n`);
}
...
async function captureOne(input: {
  ...
  try {
    ...
    try {
      ...
      await writePrivate(join(artifactRoot, "disposition/round-3-post-send-ambiguity.json"),
  ...
  await run(process.execPath, [
    "--import", "tsx", join(repositoryRoot, "scripts/mast-fresh-validation-round-3.mts"), "capture-generation",
...
async function buildJudgeManifest(artifactRoot: string, judge: string) {
  ...
  if (schedule.studyId !== STUDY_ID || !Array.isArray(schedule.entries)
    ...
    throw new Error("ROUND_3_JUDGE_SCHEDULE_INVALID");
  }
  const records = [];
  for (const entry of schedule.entries) {
    ...
    if (sourceSha256 !== entry.exactPacketSha256 || !/^[0-9a-f]{64}$/u.test(entry.exactGenerationOutputSha256)) {
      throw new Error(`ROUND_3_JUDGE_PACKET_SOURCE_MISMATCH_${entry.opaqueResponseId}`);
    }
...
async function transferJudgingPackets() {
  ...
  if (receipt.studyId !== STUDY_ID || receipt.judge !== judge || receipt.records.length !== manifest.records.length
    ...
    throw new Error("ROUND_3_JUDGE_PACKET_TRANSFER_RECEIPT_INVALID");
  }
  process.stdout.write(`${JSON.stringify({ status: "ROUND_3_JUDGE_PACKET_TRANSFER_COMPLETE", judge, packetCount: manifest.records.length, receiptSha256: sha256(await readFile(localReceipt)) })}\n`);
}
...
async function captureJudgeOne(input: { repositoryRoot: string; mastGitDirectory: string;
  ...
  if (transport.state !== "RESPONSE_COMPLETE" || transport.sourceSha256 !== record.expectedSha256
    ...
    throw new Error("ROUND_3_JUDGMENT_TRANSPORT_RECEIPT_INVALID");
  }
  await run(process.execPath, ["--import", "tsx", join(repositoryRoot, "scripts/mast-fresh-validation-round-3.mts"),
...
else if (command === "synthetic-crash-recovery") await runSyntheticCrashRecovery();
