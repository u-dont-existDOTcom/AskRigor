...
const STUDY_ID = "askrigor-mast-fresh-validation-round-4-20260919";
...
async function verifyRuntime(expectedPath) {
  ...
  for (const key of Object.keys(observed)) {
    if (!/^[0-9a-f]{64}$/u.test(expected[key] ?? "") || observed[key] !== expected[key]) {
      throw new Error(`ROUND_4_RUNTIME_HASH_DRIFT_${key.toUpperCase()}`);
    }
  }
  ...
  if (browser.ActiveState !== "active" || browser.SubState !== "running"
    ...
    throw new Error("ROUND_4_RUNTIME_SERVICE_NOT_RUNNING");
  }
  const pid = Number(browser.MainPID);
  if (!Number.isSafeInteger(pid) || pid <= 1) throw new Error("ROUND_4_BROWSER_MAIN_PID_INVALID");
...
function judgeConfig(judge) {
  ...
  throw new Error("ROUND_4_JUDGE_INVALID");
}
...
function validateJudgeManifest(manifest) {
  ...
  if (manifest?.schemaVersion !== 1 || manifest?.studyId !== STUDY_ID
    ...
    throw new Error("ROUND_4_JUDGE_MANIFEST_INVALID");
  }
  const ids = new Set();
  for (const record of manifest.records) {
    if (!/^EVAL-[0-9a-f]{24}$/u.test(record.opaqueResponseId)
      ...
      throw new Error("ROUND_4_JUDGE_MANIFEST_RECORD_INVALID");
    }
    ids.add(record.opaqueResponseId);
  }
  if (ids.size !== manifest.records.length) throw new Error("ROUND_4_JUDGE_MANIFEST_DUPLICATE");
...
async function verifyJudgeTransfer() {
  ...
  for (const source of manifest.records) {
    ...
    if (identity.sha256 !== source.expectedSha256 || identity.utf8Bytes !== source.sourceUtf8Bytes) {
      throw new Error(`ROUND_4_JUDGE_PACKET_DESTINATION_MISMATCH_${source.opaqueResponseId}`);
    }
  ...
  process.stdout.write(`${JSON.stringify({ status: "ROUND_4_JUDGE_PACKET_TRANSFER_VERIFIED", judge: config.judge, packetCount: records.length, receiptSha256: sha256(Buffer.from(canonicalJson(receipt), "utf8")) })}\n`);
  process.exit(0);
}
...
async function exactRecoveryCandidate(source, preferredConversationUrl = null, expectedRequestMessageId = null) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (safeFailureCode(error) === "ROUND_4_RECOVERY_CANDIDATE_MULTIPLE") throw error;
    }
    await sleep(1_000);
  }
...
async function runOne() {
  ...
  if (await exists(runDirectory)) {
    ...
    if (submissionAttempts.length > 1) throw new Error("ROUND_4_DUPLICATE_SEND_STATE_DETECTED");
...
async function runJudgeAttempt({ workspace, record, attempt, runtimeAttestation, expectedRuntimeHashes, config }) {
  ...
  try {
    ...
    if (source.sha256 !== record.expectedSha256 || source.utf8Bytes !== record.sourceUtf8Bytes) {
      throw new Error("ROUND_4_JUDGE_PACKET_DESTINATION_MISMATCH");
    }
    stage = "CDP_ATTACH";
    const browser = await connect();
    const { page, tabCount } = await ensureChatGptPage(browser.contexts());
...
async function runJudgeOne() {
  ...
  if (!record) throw new Error("ROUND_4_JUDGMENT_SLOT_UNKNOWN");
  ...
  throw new Error("ROUND_4_JUDGMENT_TRANSPORT_ATTEMPT_CEILING_EXHAUSTED");
}
...
async function markJudgeSealed() {
  ...
  if (!/^EVAL-[0-9a-f]{24}$/u.test(opaqueResponseId) || !/^[0-9a-f]{64}$/u.test(captureSha256)) {
    throw new Error("ROUND_4_JUDGMENT_SEAL_ARGUMENT_INVALID");
  }
  const runDirectory = join(workspace, "runs", opaqueResponseId);
  ...
  if (ready.responseSha256 !== sha256(response)) throw new Error("ROUND_4_JUDGMENT_SEAL_RESPONSE_MISMATCH");
...
async function runtimeVerification() {
  ...
  process.stdout.write(canonicalJson({ status: "ROUND_4_RUNTIME_VERIFIED", ...receipt }));
  process.exit(0);
}
