...
export function createLessonIncidentActionRoute(
  ...
  return Object.freeze({
    ...
    async handle({ body }: ActionRequestContext) {
      ...
      if (!parsedInput.success) return invalidIncident("invalid_input");
      try {
        ...
        if (error instanceof LessonIncidentVaultIntegrityError) {
          return invalidIncident("privacy_or_security_boundary");
        }
        return {
          status: 503,
          body: {
...
function invalidIncident(reasonCode: "invalid_input" | "privacy_or_security_boundary") {
  return {
    status: 422,
    body: {
